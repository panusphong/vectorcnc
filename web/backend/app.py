...
def _wrap_fit_outer(full, rec, trim_width_cm, trim_dir):
    ...
    if g <= 0.05 or full is None or full.is_empty:
        return full
    # ══════════════════════════════════════════════════════════════════
    # ⚠️ หดครั้งเดียวไม่พอ (เคสจริง 2026-08-09: กรอก 230 ได้ 230.8)
    #    ที่ 'มุมแหลม' เช่นปลายฟัน/หางตัวอักษร การ buffer แบบมุมตัดตรง (mitre)
    #    จะดันเดือยออกไปไกลกว่าความกว้างคิ้ว -> หดเข้าเท่ากันทุกด้านจึงชดเชยไม่ครบ
    # ✅ วิธี: หด -> 'จำลองใส่คิ้วกลับ' -> วัดของจริง -> เกินเท่าไหร่ก็หดเพิ่ม วนจนตรง
    #    (จำลองด้วยพารามิเตอร์เดียวกับ _mbuf เป๊ะ ๆ ตัวเลขจึงตรงกับที่ผลิตจริง)
    # ══════════════════════════════════════════════════════════════════
    def _buf(geo, d):
        return geo.buffer(d, join_style=2, mitre_limit=4.0, resolution=12)

    try:
        _b = full.bounds
        _tw = _b[2] - _b[0]; _th = _b[3] - _b[1]      # 🎯 เป้า = กรอบงานจริง = ขนาดที่ผู้ใช้กรอก
        best = None
        d = g
        for _ in range(5):
            s = _buf(full, -d)
            if s is None or s.is_empty or s.area < full.area * 0.25:
                break                                  # หดจนเหลือน้อยผิดปกติ -> หยุด ใช้ค่าที่ดีที่สุดที่มี
            best = s
            o = _buf(s, g)                             # คิ้วที่จะงอกกลับออกมาจริง
            if o is None or o.is_empty:
                break
            ob = o.bounds
            over = max(ob[2] - ob[0] - _tw, ob[3] - ob[1] - _th)
            if over <= 0.2:                            # ตรงเป้าภายใน 0.2 มม. -> พอ
                break
            d += over / 2.0                            # เกินเท่าไหร่ หดเพิ่มครึ่งหนึ่ง (เกินทั้งสองข้าง)
        return best if (best is not None and not best.is_empty) else full
    except Exception:
        return full
