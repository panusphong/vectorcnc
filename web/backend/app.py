...
BUILD_TAG = "2026-08-17-planar-all"
...
# ══════════════════════════════════════════════════════════════════
# 🏷️ บอกให้เห็นบนหน้าเว็บว่า "ตอนนี้ระบบวิ่งอยู่บนแพลนอะไร + เวอร์ชันไหน"
#
# ทำไมต้องวัดเอง ไม่อ่านจากไฟล์อย่างเดียว:
#   Render ไม่ได้ส่งชื่อแพลนมาทาง env เลย · ส่วน render.yaml เป็นแค่ 'สิ่งที่เขียนไว้'
#   เคสจริง (2026-08-09): ไฟล์เขียน plan: free ทั้งที่จ่าย Pro อยู่ — ถ้าเชื่อไฟล์ก็อ่านผิด
# ✅ วัดของจริงจากเครื่อง (แรม + จำนวน CPU) แล้วเทียบกับตารางแพลนของ Render
#    ถ้า 'ที่วัดได้' กับ 'ที่เขียนในไฟล์' ไม่ตรงกัน -> เตือนทันที เพราะแปลว่าถ้า sync จะโดนเปลี่ยนแพลน
# ══════════════════════════════════════════════════════════════════
_RENDER_TIERS = [                      # (ชื่อ, แรม GB, CPU)
    ("Free", 0.5, 0.1), ("Starter", 0.5, 0.5), ("Standard", 2, 1),
    ("Pro", 4, 2), ("Pro Plus", 8, 4), ("Pro Max", 16, 4), ("Pro Ultra", 32, 8),
]


def _host_specs():
    """แรม/ซีพียู ที่ 'ได้จริง' — อ่านจากโควตา cgroup ก่อน (ในคอนเทนเนอร์ /proc บอกของเครื่องแม่)"""
    ram = 0.0
    for pth in ("/sys/fs/cgroup/memory.max",
                "/sys/fs/cgroup/memory/memory.limit_in_bytes"):
        try:
            v = open(pth).read().strip()
            if v.isdigit() and int(v) < (1 << 50):
                ram = int(v) / (1024.0 ** 3); break
        except Exception:
            pass
    if ram <= 0:
        try:
            for ln in open("/proc/meminfo"):
                if ln.startswith("MemTotal:"):
                    ram = float(ln.split()[1]) / (1024.0 * 1024.0); break
        except Exception:
            pass
    cpu = 0.0
    try:
    ...
    return round(ram, 2), round(cpu, 2)


def _plan_declared():
    """อ่าน plan ที่ 'เขียนไว้' ใน render.yaml (ไฟล์นี้คือตัวจริงเมื่อเป็น Blueprint managed)"""
    for d in (os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
              os.getcwd(), "/app", "/"):
        f = os.path.join(d, "render.yaml")
        try:
            ...
            for ln in open(f, encoding="utf-8"):
                ...
                if t.startswith("plan:") and not plan:
                    plan = t.split(":", 1)[1].strip()
                if t.startswith("disk:") or t.startswith("mountPath:"):
                    disk = True
            return plan, disk, f
        except Exception:
            continue
...
_PLAN_CACHE = {"t": 0.0, "d": None}


def _plan_from_api():
    """🎯 ถาม Render ตรง ๆ ว่า service นี้ถูกปล่อยด้วยแพลนอะไร — คำตอบที่เชื่อได้ที่สุด

    ต้องมี 2 อย่างเป็น env บน Render:
      RENDER_API_KEY  = คีย์จาก Render → Account Settings → API Keys
      RENDER_SERVICE_ID = Render ใส่ให้เองอยู่แล้ว (ของบริการนี้คือ srv-d935jclaeets73b614tg)
    ถ้าไม่มีคีย์ -> คืน None แล้วไปใช้ 'ค่าที่วัดจากเครื่อง' แทน (ใกล้เคียงแต่เป็นการเดา)
    """
    key = (os.environ.get("RENDER_API_KEY", "") or "").strip()
    sid = (os.environ.get("RENDER_SERVICE_ID", "") or "").strip()
    if not key or not sid:
        return None
    import time as _t
    if _PLAN_CACHE["d"] and (_t.time() - _PLAN_CACHE["t"]) < 600:
        return _PLAN_CACHE["d"]
    try:
        ...
        det = j.get("serviceDetails") or {}
        d = {"plan": str(det.get("plan") or "").strip(),
             "region": str(det.get("region") or ""),
             "instances": det.get("numInstances"),
             "suspended": j.get("suspended", ""),
             "name": j.get("name", ""), "src": "render-api"}
        disk = det.get("disk") or {}
        if disk:
            d["disk_name"] = disk.get("name", "")
            d["disk_gb"] = disk.get("sizeGB", "")
            d["disk_mount"] = disk.get("mountPath", "")
        _PLAN_CACHE["t"] = _t.time(); _PLAN_CACHE["d"] = d
        return d
    except Exception as e:
        print("[appinfo] ถาม Render API ไม่สำเร็จ:", repr(e)[:160], flush=True)
        return None


def _plan_info():
    ram, cpu = _host_specs()
    guess = min(_RENDER_TIERS,
                key=lambda t: abs(t[1] - ram) / max(t[1], .5) + abs(t[2] - cpu) / max(t[2], .1))
    decl, decl_disk, yml = _plan_declared()
    same = (not decl) or decl.replace(" ", "").lower() == guess[0].replace(" ", "").lower()
    warn = ""
    if decl and not same:
        warn = ("⚠️ render.yaml เขียนไว้ว่า '%s' แต่เครื่องที่วิ่งอยู่จริงคือ '%s' "
                "— ถ้า blueprint ถูก sync เมื่อไหร่ ระบบจะเปลี่ยนไปตามไฟล์" % (decl, guess[0]))
    # 🥇 ทางที่ปลอดภัยที่สุด: บอกชื่อแพลนตรง ๆ ผ่าน env PLAN_NAME
    #    ⚠️ ทำไมถึงแนะนำทางนี้ก่อน: Render API key เป็นกุญแจ 'ระดับทั้งบัญชี'
    #       (แก้แพลน/ลบ service ได้) การเอาไปฝากไว้ใน env ของเว็บเพียงเพื่อ 'โชว์ชื่อแพลน'
    #       ไม่คุ้มความเสี่ยง · ตั้ง PLAN_NAME=pro ก็จบ ไม่ต้องมีกุญแจอะไรเลย
    env_plan = (os.environ.get("PLAN_NAME", "") or "").strip()
    api = None if env_plan else _plan_from_api()
    src = "วัดจากเครื่อง (ประมาณการ)"
    plan = guess[0]
    if env_plan:
        plan = env_plan.replace("_", " ").title()
        src = "ตั้งไว้เอง (env PLAN_NAME)"
        same = (not decl) or decl.replace(" ", "").lower() == env_plan.replace(" ", "").replace("_", "").lower()
        warn = ("" if same else
                "⚠️ ตั้ง PLAN_NAME ไว้ว่า '%s' แต่ render.yaml เขียน '%s' — ควรทำให้ตรงกัน"
                % (plan, decl))
    elif api and api.get("plan"):
        # ✅ ได้คำตอบจาก Render เอง -> ใช้ตัวนี้เป็นหลัก ไม่ต้องเดาอีก
        plan = api["plan"].replace("_", " ").title().replace("Pro Plus", "Pro Plus")
        src = "Render API (ของจริง)"
        same = (not decl) or decl.replace(" ", "").lower() == api["plan"].replace(" ", "").replace("_", "").lower()
        warn = ("" if same else
                "⚠️ Render ปล่อยบริการนี้ด้วยแพลน '%s' แต่ render.yaml เขียนไว้ว่า '%s' "
                "— ถ้า blueprint ถูก sync ระบบจะเปลี่ยนไปตามไฟล์" % (plan, decl))
    return {"plan": plan, "source": src, "measured": guess[0], "api": api,
            "env_plan": env_plan,
            "ram_gb": ram, "cpu": cpu,
            "declared": decl, "yaml": yml, "match": bool(same), "warn": warn,
            "disk_declared": decl_disk, "disk_persist": _AN_PERSIST, "data_dir": _AN_DB,
...
@app.get("/api/appinfo")
def api_appinfo(request: Request):
    """🏷️ ข้อมูลป้ายหัวเว็บ — เวอร์ชัน + อัปเดตล่าสุดแบบสั้น (+ แพลนเครื่อง เฉพาะแอดมิน)"""
    out = {"ok": True, "build": BUILD_TAG, "update": BUILD_SHORT}
...
def health():
    ...
    # 🧠 แรมที่คอนเทนเนอร์นี้มีจริง — เปิดดูได้ที่ /api/health
    ...
    return {"ok": True, "service": "VectorCNC",
            ...
            "build": BUILD_TAG, "update": BUILD_SHORT, "mem": _memq,
        "build_note": "🎨 หน้าแตกชิ้นกลายเป็น ที่สร้างโลโก้แบรนด์ ครบจบในที่เดียว (2026-08-09 รอบ 9): ผู้ใช้ขอให้มองหน้านี้เป็นการสร้างโลโก้แบรนด์ก่อนไปวางกับประเภทป้าย — พิมพ์ข้อความได้อิสระ เลือกฟอนต์ ปรับรูปแบบ และสร้างพื้นหลังให้จบจากตรงนี้ ✅ เพิ่ม 2 ปุ่มในแถบเดิม ไม่มีหน้าจอใหม่ (ตามที่สั่งไว้ว่า อย่าทำหลายจุด user งงหมดแล้ว): ✍️ ใส่ข้อความ — ฟอนต์ · ความสูงเป็น ซม. · สี · ขอบรอบตัว · จัดชิด · เอียง · ระยะห่างตัวอักษร/บรรทัด ตัวอย่างที่เห็นคือของจริงที่เซิร์ฟเวอร์สร้างแล้ว ไม่ใช่การจำลองบนจอ · 🎨 ใส่พื้นหลัง — สีเดียว/ไล่สี/ไฟล์ artwork · สี่เหลี่ยม/มุมมน/วงรี · เส้นขอบ (ใช้เป็นหน้ากล่องไฟพิมพ์ UV ได้) ⚠️ เรื่องที่ต้องระวังและแก้แล้ว: (1) ในเครื่องไม่มีไฟล์ฟอนต์ไทยเลยสักตัว มีแต่ฟอนต์ลายมืออังกฤษ 49 ตัว ที่หน้าเว็บโชว์ฟอนต์ไทยได้เพราะดึงจาก Google CDN ซึ่งเอามาทำเวกเตอร์ไม่ได้ -> โหลดฟอนต์ OFL เข้ามา 32 ไฟล์ (ไทย 23 ตัว) (2) ตัวอักษรไทยวางสระ/วรรณยุกต์ซ้อนบนพยัญชนะ ถ้าเดินตามความกว้างเฉย ๆ สระจะหลุดตำแหน่ง -> ผ่าน HarfBuzz ตัวจัดวางเดียวกับที่เบราว์เซอร์ใช้ และปฏิเสธทันทีถ้าเลือกฟอนต์ที่ไม่มีตัวอักษรไทย (3) ตัวหนังสือสีขาวบนพื้นโปร่ง ทำให้ช่องรูปย่อดูเหมือนว่างเปล่า -> รูปย่อมีพื้นเทารอง (บนกระดานยังโปร่งเหมือนเดิม) (4) แผ่นพื้นหลังต้องอยู่ชั้นล่างสุดทั้งบนจอและในไฟล์ที่รวมออกไป และชิ้นใหม่ต้องวางต่อใต้ชิ้นล่างสุด ไม่ใช่กองที่มุมซ้ายบน 📐 เส้นตัวอักษรดึงจากเส้นโค้งเบซิเยร์ในไฟล์ฟอนต์ตรง ๆ ไม่ผ่านภาพเลยสักขั้น -> รวมเป็นเวกเตอร์แท้ต่อได้ทันที ไม่ต้อง trace | 🎚️ ปุ่ม ความเนียนของเส้น ใช้ได้จริงแล้ว (2026-08-09 รอบ 8): ผู้ใช้ทักว่าเส้นเวกเตอร์ยังเป็นลอน ปรับปุ่มแล้วไม่เห็นต่าง ⚠️ ตรวจแล้วพบว่า ปุ่มนั้นไม่ได้คุมสิ่งที่ผู้ใช้เห็นเลย — มันคุมแค่การเกลาสนามสี (sigma) แต่ลอนที่ตาเห็นเกิดที่แนวจุดคอนทัวร์ ซึ่งคุมด้วยงบเกลาแนวจุดคนละตัวกัน ✅ ผูกปุ่มเข้ากับงบเกลาแนวจุด ตัวคูณ 0.45/1.0/2.0/3.4 เพดาน 1/2.5/5/8 px ตามระดับที่เลือก วัดจริง (สมดุล -> เนียนมาก): test03 ลอน 6.61 -> 4.10 จุด 4113 -> 2759 · test06 5.22 -> 4.95 ⚠️ ทางที่ลองแล้วถอยออก: เอาเกณฑ์ลอนไปใส่ในการเกลาสนามสี ทำให้ภาพสะอาดพังหนัก (counters 3.27 -> 4.90 · corners 2.53 -> 4.26) เพราะรูปทรงสั้นให้ค่าลอนสูงหลอก -> ถอยออก ค่าเริ่มต้นจึงไม่เปลี่ยนเลย ΔE 5.54 ลอน 3.39 จุด 1033 เท่าเดิม · ผ่านชุดตรวจ 13/13 | 👤 สถิติรู้แล้วว่าใครเข้ามาใช้ (2026-08-09 รอบ 7): ผู้ใช้เข้าผ่าน CRM Hub แต่สถิติยังขึ้น guest ทุกแถว ⚠️ สาวจนถึงต้นตอ เป็นลูกโซ่ 3 ต่อ: (1) CRM Hub เปิดลิงก์ /app?t=ตั๋ว มาให้ (2) gate() เก็บตั๋วใส่ localStorage แล้ว **ลบตั๋วออกจาก URL ทันที** (ตั้งใจทำ กันตั๋วหลุดตอนแชร์ลิงก์) ช่องที่อยู่จึงเหลือแค่ /app — ไม่ใช่บั๊ก แต่เป็นการกันตั๋วรั่ว (3) ⚠️ **ตัวส่งสถิติไม่เคยแนบตั๋วไปด้วยเลย** พอตั๋วไม่อยู่ใน URL แล้ว เซิร์ฟเวอร์จึงไม่มีทางรู้ว่าใคร -> บันทึกเป็น guest ทุกครั้ง ✅ แก้ 2 จุด: หน้าเว็บแนบ header X-User-Token จาก localStorage ไปกับทุก event (ถ้ามีตั๋วจะใช้ fetch แทน sendBeacon เพราะ sendBeacon แนบ header ไม่ได้) · เซิร์ฟเวอร์อ่านอีเมลจากตั๋วมาเติมให้เองเมื่อหน้าเว็บส่ง guest มา ⚠️ จุดที่พลาดตอนแก้: เดาว่าตั๋วเก็บชื่อไว้ที่คีย์ u แต่ของจริงคือ e (ดู auth.py) ทดสอบจริงด้วยตั๋วจริงแล้วขึ้นชื่ออีเมลถูกต้อง ไม่ใช่ guest | 🧹 เคลียร์ 3 เรื่องที่ผู้ใช้ทัก (2026-08-09 รอบ 6): (1) หน้าสถิติขึ้นเตือนสีเหลือง ในเครื่อง (รีเซ็ตตอน deploy) ตลอด ⚠️ ต้นเหตุ: หน้าเว็บดูแค่ว่า ข้อมูลมาจาก Google Sheet หรือไม่ ถ้าไม่ใช่ก็เตือนทันที ทั้งที่ถ้าต่อดิสก์ถาวรไว้ ข้อมูลก็ไม่หายอยู่แล้ว -> ผู้ใช้ตกใจฟรี → เปลี่ยนให้ดูสถานะจริง 3 แบบ: ชีต(ถาวร) / ดิสก์ถาวร(ถาวร) / ชั่วคราว(เตือน) และโชว์จำนวนที่ค้างส่งเข้าชีตด้วย (2) สถิติขึ้นชื่อผู้ใช้เป็น guest ทั้งที่เป็นเจ้าของเอง ⚠️ ต้นเหตุ: ชื่อมาจากฝั่งหน้าเว็บอย่างเดียว เปิดเว็บตรง ๆ ไม่ผ่าน CRM Hub จึงไม่มีชื่อจะส่ง → ฝั่งเซิร์ฟเวอร์อ่านชื่อจากตั๋ว SSO เติมให้เองเมื่อหน้าเว็บส่ง guest มา (3) ปิดปุ่ม ทดลองฟรี บนหน้า landing ไว้ก่อนตามที่สั่ง (ซ่อนด้วย display:none + คอมเมนต์บอกวิธีเปิดกลับ) ปุ่ม เข้าใช้งาน ปกติยังอยู่ครบ | 🏷️ ป้ายบอกแพลน + เวอร์ชัน บนหัวเว็บ (2026-08-09 รอบ 5): ผู้ใช้อยากรู้จากหน้าเว็บเลยว่าแอปวิ่งอยู่บนแพลนอะไร และเวอร์ชันที่เปิดอยู่อัปเดตเรื่องอะไร · เพิ่มป้ายบนแถบเมนู กดแล้วเห็นรายละเอียด (แพลน · แรม · ซีพียู · ที่เก็บสถิติถาวรไหม · เวอร์ชัน · สรุปอัปเดต) · รายละเอียดแพลนโชว์เฉพาะแอดมิน คนทั่วไปเห็นแค่เลขเวอร์ชัน · ⚠️ Render ไม่ส่งชื่อแพลนมาทาง env เลย และ render.yaml เป็นแค่สิ่งที่เขียนไว้ (เคสจริง: ไฟล์เขียน free ทั้งที่จ่าย Pro) จึงทำ 2 ทาง: (ก) ถาม Render REST API ตรง ๆ = คำตอบจริง ต้องตั้ง env RENDER_API_KEY (ข) ถ้าไม่มีคีย์ วัดแรม/ซีพียูจริงจาก cgroup แล้วเทียบตารางแพลน = ประมาณการ พร้อมบอกชัดว่าเป็นการประมาณ · ถ้าแพลนจริงกับที่เขียนใน render.yaml ไม่ตรงกัน ป้ายเปลี่ยนเป็นสีเหลืองเตือนทันที (กันเคสโดนลดแพลนเงียบ ๆ ตอน blueprint sync) | 🔒 สถิติการเข้าเว็บเก็บถาวรจริง (2026-08-09 รอบ 4): ⚠️ ตรวจแล้วพบสองเรื่องที่ทำให้สถิติหายได้: (1) render.yaml เขียน plan: free ไว้ ทั้งที่ของจริงบน Render เป็น Pro และบริการเป็น Blueprint managed = ไฟล์นี้คือตัวจริงที่คุมค่า ถ้าถูก sync เมื่อไหร่จะโดนลดกลับเป็น free (ดิสก์หลุด · แรมเหลือ 512MB · เครื่องสลีป) → แก้เป็น plan: pro + เพิ่มดิสก์ถาวร 5 GB ที่ /var/data + ตั้ง DATA_DIR ให้ชัด (2) การยิงสถิติเข้า Google Sheet เป็นแบบ ยิงแล้วลืม — ถ้าพลาด (เน็ตสะดุด · Apps Script ตื่นช้า · เครื่องรีสตาร์ตตัดเธรดกลางคัน) event นั้นหายถาวร ไม่มีใครรู้ ไม่มีการลองใหม่ → เพิ่มคิวค้างส่ง: พลาดเมื่อไหร่เก็บเข้าคิวทันที ลองส่งใหม่ให้เองตอนบูตและทุกครั้งที่มี event ใหม่ · เขียนคิวแบบเขียนไฟล์ชั่วคราวแล้วสลับ กันไฟดับกลางคันแล้วคิวหาย · เพิ่มรายงานสุขภาพในทุกคำตอบของ /api/stats (ถาวรอยู่ไหม · ค้างกี่รายการ · พลาดล่าสุดเพราะอะไร) · เพิ่ม /api/stats-health?test=1 สำหรับแอดมิน กดแล้วเขียนแถวทดสอบเข้าชีตแบบรอผลจริง แล้วบอกสาเหตุถ้าพลาด (ของเดิมถ้าชีตล่มจะไม่มีทางรู้จนกว่าจะเปิดดูแล้วพบว่าตัวเลขไม่ขึ้น) · ทดสอบจริง: จำลองยิงชีตไม่ผ่าน -> event ถูกเก็บเข้าคิวครบ ไม่หายสักรายการ | ✂️ ตัดพื้นหลังออกจากชิ้นงานให้อัตโนมัติ (2026-08-09 รอบ 3): ผู้ใช้ทักว่ารูปฟันติดพื้นหลังมาด้วย และขอให้ระบบคิดให้เอง ไม่ใช่ให้ผู้ใช้ไปไล่ลบ ⚠️ ต้นเหตุ: รูปย่อของแต่ละชิ้นสร้างด้วยวิธี เรนเดอร์หน้ากระดาษทั้งหน้าแล้วครอบกรอบ ไฟล์ที่มีสี่เหลี่ยมพื้นหลัง (เจอบ่อยมากในไฟล์ลูกค้า) ทุกชิ้นจึงกลายเป็น ตัวอักษรบนแผ่นสีเขียว ทั้งที่ตัวชิ้นจริงไม่มีพื้นหลังติดอยู่เลย ✅ เปลี่ยนเป็นวาดเฉพาะเส้นของชิ้นนั้นชิ้นเดียวลงบนพื้นโปร่ง และข้ามรูปที่คลุมเกิน 72% ของหน้า (= พื้นหลัง) ทิ้ง ผลกับไฟล์จริง YN smile: 17 ชิ้นเวกเตอร์ พื้นหลังหายหมดทุกชิ้น รูปฟันขาวออกมาสะอาดบนพื้นโปร่ง ถ้าชิ้นไหนไม่มีเส้นให้วาด ระบบถอยไปใช้วิธีเดิมอัตโนมัติ ไฟล์เดิมจึงไม่มีอะไรพัง | 🩹 ไฟล์ .svg เข้าหน้าแตกชิ้นงานได้แล้ว — แก้ที่จุดเดียว ไม่สร้างหน้าใหม่ (2026-08-09 รอบ 2): ผู้ใช้ทักว่าหน้าที่อลิซทำเพิ่มมันใช้ยาก และมีหลายจุดจน user งง — ถูกแล้ว ที่จริงหน้า แตกชิ้น+เลือก+จัดวาง ของเดิมทำสิ่งที่ต้องการได้เกือบครบอยู่แล้ว (ลากย้าย · ย่อขยาย · จัดกึ่งกลาง · วัดระยะ · แยกละเอียดขึ้น · รวมมากขึ้น · คลังชิ้นงาน · รวมเป็นแบบไปต่อประเภทป้าย) ⚠️ ต้นเหตุจริงคือ **เส้นทางไฟล์** — .ai/.pdf/.eps ถูกส่งไปทางเวกเตอร์แท้ แต่ **.svg ถูกโยนไปทางภาพ** ทั้งโลโก้เลยกลายเป็นชิ้นเดียวและเป็นภาพ แตกชิ้นไม่ได้ (เคสจริง YN smile: ในไฟล์มี 58 รูปทรง แต่จอขึ้น 1 ชิ้น ภาพ) ✅ แก้ 2 จุด: (1) /api/extract-assets รับ .svg แล้วแปลงเป็น PDF ด้วย cairosvg ก่อน (จัดการ transform/group/mask ครบ) (2) หน้าเว็บส่ง .svg ไปเส้นทางเดียวกับ .ai/.pdf ผลกับไฟล์จริง: แตกได้ 20 ชิ้น เป็นเวกเตอร์แท้ทั้งหมด ครบ 100% ไม่มีชิ้นหาย และ **โลโก้ฟันขาวบนพื้นขาวออกมาเป็นชิ้นของตัวเอง** (สายงานเดิมอ่านจาก path ไม่ได้อ่านจากสี ปัญหาขาวบนขาวจึงไม่มีตั้งแต่แรก) | 🧰 โมดูล vectora_vector.py ยังอยู่เป็นเครื่องมือหลังบ้าน (อ่าน .svg/.ai/.pdf/.eps · แตกชิ้น · จัดกลุ่มแบบที่คนมองออก · ประกอบร่างใหม่หน่วยมิลลิเมตร · ขอบกล่องล้อมตามทรง · หน้าป้ายพิมพ์ UV ครอบตามทรงจริง) ไว้ต่อยอดในหน้าเดิม ไม่ได้เปิดเป็นหน้าแยก | 🧩 ออกแบบป้ายจากไฟล์เวกเตอร์ของลูกค้า (2026-08-09): เคสจริง YN smile — ลูกค้าส่ง .svg มา ทำได้ 2 งานจากไฟล์เดียว (ไดคัทอะคริลิค 8 มม. 130 cm + กล่องไฟ 230x85 cm) แต่ระบบเดิมรับไม่ได้ เพราะหาขอบป้ายจาก **ความต่างของสีในภาพ** ซึ่งพังสองชั้นกับไฟล์นี้: มีสี่เหลี่ยมพื้นหลังคลุมทั้งใบ (จับได้แค่กรอบสี่เหลี่ยม) และโลโก้รูปฟันเป็น **ขาวบนขาว** (ไม่มีความต่างสีให้จับ) → เลิกมองเป็นภาพ อ่าน path จริงจากไฟล์แทน สีไม่เกี่ยวเลย | โมดูลใหม่ vectora_vector.py: อ่าน .svg/.ai/.pdf/.eps ผ่าน PDF+PyMuPDF (ได้พิกัดหลัง transform + สีจริงของทุกรูป) · แตกเป็นชิ้นแยกทีละชิ้นพร้อมรูข้างใน · ตรวจจับพื้นหลังอัตโนมัติ (สี่เหลี่ยมคลุม >82% ของหน้า) · ประกอบร่างใหม่ได้ (ย้าย/ย่อขยาย/พลิก ทีละชิ้น หน่วยมิลลิเมตรจริง) · ทุกชิ้นมีบทบาท 3 แบบ: กำหนดทรงกล่อง / แค่พิมพ์บนหน้าป้าย / ทั้งสอง — ตัวนี้ทำให้ย้ายฟันไปไหนก็ได้แล้วขอบกล่องอ้อมตาม · หน้าป้ายออกแบบเองได้: แถบสีหลายแถบ + ข้อความหลายก้อนแยกกัน + พื้นหลังจากไฟล์ artwork · งานพิมพ์ถูก **ครอบตามทรงกล่องจริง** ไม่ใช่ครอบสี่เหลี่ยม (ไม่งั้นพิมพ์เลยขอบอะคริลิคตรงรอยอ้อมฟัน) · ออก 2 ไฟล์แยกหน้าที่: เส้นตัด (SVG/DXF/PDF หน่วยมม. แกน Y ถูกทางเครื่องตัด) กับ งานพิมพ์ UV (SVG/PDF) | ⚠️ บั๊กที่เจอตอนทดสอบกับไฟล์จริงและแก้แล้ว 3 ตัว: (1) เรียงชั้นตามขนาดพื้นที่ (มรดกจากงานไล่เส้นจากภาพ) ทำให้ฟันขาวรวม 2.39 ล้าน > พื้นหลัง 2.25 ล้าน ถูกดันไปอยู่หลังแล้วหายทั้งตัว (2) รวมทุกอย่างที่สีเดียวกันข้ามลำดับ พังด้วยเหตุผลเดียวกัน → คงลำดับหน้า-หลังตามไฟล์เป๊ะ (3) ไดคัทถูกยุบเหลือก้อนเดียว → แยกโหมด เก็บครบทุกชิ้น | วัดกับไฟล์จริง: 58 ชิ้น · พื้นหลัง 2 ชิ้นตรวจเจอเอง · ไดคัท 44 ชิ้นตัด · กล่องไฟประกอบออกมา 230.0 x 85.0 cm ตรงใบสั่งงาน | 🖨️ รับไฟล์เวกเตอร์เข้าเมนูแปลงภาพด้วย — แต่ไม่ trace ใหม่ (เป็นเวกเตอร์อยู่แล้ว ไล่เส้นซ้ำมีแต่เสียความคม) | ✨ เส้นเนียนขึ้นจริง + สีตรงขึ้นจริง (2026-08-08): ผู้ใช้ทักว่าความคมยังสู้ไม่ได้ · วัดแล้วเจอต้นเหตุสามข้อ ไม่ใช่เรื่องความละเอียด ① ตัวฟิตเส้นโค้งของเดิม ลดจุดแล้วร้อยเส้นผ่านจุดที่เหลือ (Catmull-Rom) ได้ 1 ท่อนโค้งต่อ 1 ช่วงจุด เส้นเลยส่ายและจุดเยอะมาก (ไฟล์จริง test03: 48 รูป 8,662 จุด) → เปลี่ยนเป็นวิธีหากำลังสองน้อยที่สุด + ผ่าตรงจุดที่คลาดมากสุด (Schneider) โค้งเดียวกินทางยาวได้ ต่อทิศทางที่รอยต่อ ไม่มีรอยหัก และรับประกันความคลาดในตัวเอง (วงกลม 400 จุด -> 8 ท่อน คลาด 0.29 px) ② เกลาแนวจุดแบบไม่หด (Taubin) ภายในงบความคลาดที่วัดจากความฟุ้งของขอบและความหนาเส้นจริง ลบคลื่นจากรอยบีบอัดโดยไม่กินรูปทรง หยุดเองทันทีที่เนียนพอ ภาพสะอาดจึงแทบไม่ถูกแตะ ③ ⚠️ บั๊กสีที่ซ่อนมาแต่ต้น: OpenCV เก็บ Lab แบบ 8 บิตโดยคูณช่องความสว่างไว้ 2.55 เท่า โค้ดเดิมเอาไปวัดระยะตรง ๆ = ให้น้ำหนักความสว่างเกินจริง 2.55 เท่า จานสีจึงเพี้ยนจากเนื้อสี → แปลงกลับเป็นสเกลจริงก่อนวัดทุกครั้ง │ วัดกับชุดตรวจ 11 ไฟล์: ค่าคลาดสี 5.80 -> 5.54 · ความส่ายของเส้น 4.34 -> 3.39 · มุมหัก 0.28 -> 0.18 · จำนวนจุด 1,990 -> 1,033 (เล็กลงครึ่งหนึ่ง) │ เทียบกับคำตอบที่ถูกจริง (ไอคอนถูกขยาย 8 เท่า): ค่าคลาดสี 6.56 -> 4.19 ดีขึ้น 36% │ ผ่านชุดตรวจ 13/13 · ค่าคงที่ทุกตัวมาจากการกวาดหาบนชุดตรวจ ไม่ได้ตั้งเอาเอง | 🔍 เห็นของจริงตั้งแต่หน้าจอ (2026-08-07 รอบ 7): ผู้ใช้ทักว่าภาพเข้าหยาบแค่ไหนต้องเห็นแบบนั้นเลย · ตรวจแล้วเจอว่าปัญหาอยู่ที่หน้าจอ ไม่ใช่เครื่องแปลง — กล่องเทียบกว้างราว 720 จุด แต่ไฟล์ผู้ใช้กว้าง 2000-4000 พิกเซล เบราว์เซอร์ย่อลงมาแสดงแล้วเกลี่ยพิกเซลให้เนียนเอง ความหยาบของไฟล์เดิมเลยหายไปจากจอ สองฝั่งดูเหมือนกันหมด · แก้ที่หน้าจออย่างเดียว ไม่แตะเครื่องแปลงเลย: เพิ่มชั้นซูมให้ทั้งสองฝั่งใช้ค่าเดียวกันเสมอ (เทียบจุดต่อจุดได้จริง) · เริ่มต้นที่ 1:1 = 1 พิกเซลในไฟล์ ต่อ 1 จุดบนจอ · บังคับ nearest-neighbor ห้ามเบราว์เซอร์เกลี่ยพิกเซลต้นฉบับ · เลือกได้ พอดีจอ / 1:1 / 2x / 4x / 8x · ลากในภาพเพื่อเลื่อนดู ลากแถบขาวเพื่อเลื่อนเส้นเทียบ (คนละการลาก ไม่ชนกัน) · ผลลัพธ์: ต้นฉบับโชว์บล็อกพิกเซลกับรอยบีบอัดตามจริง เวกเตอร์คมทุกระดับซูม เห็นชัดว่าระบบทำอะไรให้ | 🎯 ความตรงตามไฟล์มาก่อน (2026-08-07 รอบ 6): ผู้ใช้ทักว่ารายละเอียดหายอีก ตรวจแล้วยืนยันว่า **ไม่มีการย่อภาพเหลืออยู่เลย** (cv2.resize เหลือ 2 บรรทัดที่อยู่ในตัววัดอย่างเดียว ไม่ได้ต่อเข้าสายการแปลง · scale=1.0000 ทุกไฟล์) ตัวที่กินรายละเอียดคือ 'การเกลา' ที่ระบบตั้งให้เองตามขนาดบล็อกพิกเซล · แก้เป็นเริ่มที่ ไม่เกลาเลย (sig0=0) เสมอ จะเกลาก็ต่อเมื่อวัดได้ว่าเส้นหยาบจริงเท่านั้น · เปลี่ยนป้ายปุ่มเป็น ตรงตามไฟล์ / สมดุล / เนียนมาก ให้ผู้ใช้เห็นชัดว่ากำลังแลกอะไรกับอะไร · วัดจริงกับโลโก้ Ginger: σ 0.96 -> 0.30 · รอยหยักบนแขนตัว F และช่องในตัว A กลับมาครบ · ผ่านชุดตรวจ 10/10 สังเคราะห์ + 9/9 ไฟล์จริง | 📐 มาตรฐานขาเข้าชัดเจน + ชุดตรวจสังเคราะห์ (2026-08-07 รอบ 5): เลิกไล่แก้ทีละไฟล์ · ประกาศมาตรฐานตายตัว: รับ 64x64 ถึง 4000x4000 px (16 ล้านพิกเซล) ไฟล์ไม่เกิน 30 MB · แปลงที่ความละเอียดเดิม 100% ไม่ย่อไม่ขยาย · เกินเพดาน = ไม่แปลง บอกให้ผู้ใช้ย่อเอง (ไม่แอบย่อให้) · เพิ่ม vectora_corpus.py สร้างภาพทดสอบสังเคราะห์ 10 แบบที่รู้คำตอบถูกอยู่แล้ว (วงแหวนซ้อน · ช่องตัวอักษร · เส้นบาง 2px · รูจิ๋ว · 12 สีติดกัน · มุมแหลม · ถูกขยาย · จุดรบกวน · โปร่ง · ไล่เฉด) · ชุดตรวจจับบั๊กจริงได้ 3 ตัวที่ตามองไม่เห็น: (1) เกณฑ์พื้นที่ไปตัดรูจริงขนาด 27px ในตัว & % (2) มุมแหลมบวมเพราะ buffer แบบ mitre (3) **ตัวฟิตเบซิเยร์ทำให้ด้านตรงโป่งออกเป็นส่วนโค้ง 3.1%** · แก้ข้อ 3 ด้วย fit_bounded: ฟิตแล้ววัดกลับ ถ้าเบี่ยงเกินเกณฑ์ให้ผ่าครึ่งฟิตใหม่ = รับประกันได้ว่าเส้นเวกเตอร์ไม่เบี่ยงจากขอบที่ตรวจได้เกินค่าที่ตั้งไว้ · ผลตอนนี้ผ่าน 10/10 ทั้งชุดสังเคราะห์และไฟล์จริง | 🚫 ไม่ย่อภาพต้นฉบับเด็ดขาด (2026-08-07 รอบ 4 — ผู้ใช้ทักซ้ำ ถูกต้องแล้ว): รอบก่อนอลิซเอาการย่อภาพกลับมาใช้อีก แค่เปลี่ยนเหตุผลเป็น 'เนื้อจริงเล็กกว่าที่ไฟล์บอก' ซึ่งก็คือการลดความละเอียดภาพผู้ใช้อยู่ดี · ตอนนี้เอาออกหมด คำนวณที่ความละเอียดเต็มเสมอ · ค่าที่วัดได้ (ขนาดบล็อกพิกเซลจริง F) เอาไปตั้ง 'ความแรงการเกลา + เกณฑ์เศษ + ความถี่จุด' เท่านั้น · แก้บั๊กถมช่องในตัวอักษร A: เลิกตัดรูด้วยขนาด เปลี่ยนเป็นดูว่าข้างในรูเป็นสีอื่นจริงไหม (รูจริงค่าสนามใกล้ 0 · รูปลอมจากรอยบีบอัดค้างแถว 0.4) รูจริงจึงไม่มีทางโดนถม · เพิ่มชุดตรวจถาวร vectora_selftest.py วัด 4 อย่าง: ถมทับ · กินหาย · จำนวนรู · ความหยึกหยัก ผ่านครบ 10/10 ไฟล์ทดสอบ · เทียบ vtracer: ไฟล์ที่ถูกขยายมา เนียนกว่า 22 เท่า ไฟล์เล็กกว่า 22 เท่า | 🔎 ความละเอียดจริง (2026-08-07 รอบ 3 · แก้จากไฟล์ทดสอบจริงของผู้ใช้ 3 ไฟล์): ไฟล์บอก 2362x2362 px แต่ย่อ 8 เท่าแล้วขยายกลับภาพแทบไม่ต่าง = เนื้อจริงมีราว 295 px เท่านั้น เครื่องแปลงจึงเดินเส้นไต่ขั้นบันไดของบล็อกพิกเซลกว้าง 8 px ออกมาเป็นขอบหยักเป็นคลื่น · แก้เป็นวัดความละเอียดจริงก่อน แล้วคำนวณที่ระดับนั้น · วัดความคลาดเฉพาะพิกเซลที่เป็นขอบ (วัดเฉลี่ยทั้งภาพไม่ได้ พื้นที่ว่างกลบหมด) · มีเพดานที่สอง: ห้ามย่อจนเส้นบางสุดในภาพเหลือน้อยกว่า 8 px (ไม่งั้นตัวอักษรแตกเป็นก้อน) · ตัดรูจิ๋วที่เกิดจากรอยด่างการบีบอัดในเนื้อเส้น (รูจริงในงานออกแบบใหญ่กว่า 8 เท่าเสมอ) · วัดจริง 3 ไฟล์: หมู ÷4 · Baron's ÷1.2 (เส้นบาง 10px) · Ginger ÷1.8 · ทุกไฟล์เส้นเนียน รายละเอียดครบ · แจ้งผู้ใช้บนหน้าจอว่าไฟล์ถูกขยายมาและเนื้อจริงเท่าไหร่ | ✨ ความเนียนของเส้น (2026-08-07 รอบ 2): ค่าเกลาเดิมตั้งตายตัว จูนไว้กับภาพ 250-500 px พอเจอภาพ 2362 px มันอ่อนลง 5 เท่า เส้นเลยตามรอยหยึกหยักของไฟล์จนเป็นขั้นบันได · แก้เป็นวัดความหยาบจริง (จำนวนมุมหักต่อ 100 px) แล้วไล่เพิ่มความเกลาทีละขั้นจนพอ ภาพสะอาดจึงไม่โดนเกลาเลย (σ=0) ส่วนภาพหยาบไต่ขึ้นไปเอง · เกลาที่สนามความเป็นสีไม่ใช่ที่เส้น (เบลอเกาส์เซียนไม่ขยับระดับ 0.5 = ตำแหน่งขอบไม่เลื่อน) · ประเมินจุดรบกวนแล้วกรองด้วย bilateral ก่อนแยกสี เฉพาะภาพที่มีจริง · ล็อกเมล็ด cv2.kmeans แล้ว ผลลัพธ์คงที่ทุกครั้ง (เดิมกดแปลงซ้ำได้จานสีคนละชุด) · ชดเชยระยะดันขอบตามความแรงที่เกลา กันเส้นขาวคั่นสี (1,313 px -> 0) · วัดจริง: ลายเส้น 2362px หยาบมาก 29,665 รูป -> 17 รูป · RMS 9.10 (ดีที่สุดที่เคยได้) | 🔧 แก้ข้อบกพร่องร้ายแรง 2026-08-07: เครื่องแปลงเคย **ย่อภาพผู้ใช้เหลือ 1600 px เงียบ ๆ** ก่อนคำนวณ (ภาพ 2362 px โดนทิ้งรายละเอียด 32%) เหตุเพราะเก็บระยะถึงทุกสีไว้ทั้งก้อน = k×H×W กินแรมมหาศาล · แก้เป็นเก็บแค่อันดับ 1-2 (แรมคงที่ไม่ขึ้นกับจำนวนสี) + Lab เป็น uint8 + สนามสีเป็น float32 · ผลลัพธ์: ทำงานที่ความละเอียดเต็มเสมอ · ภาพ 2362px 7.9 วิ → 1.6 วิ แรม 294 MB · RMS เทียบต้นฉบับ 11.11 → 9.10 · เกณฑ์ตัดเศษเล็กโตตามขนาดภาพแล้ว (ลายเส้นเคยได้ 1,054 รูปจากจุดฝุ่น JPEG → 17 รูป) · ถ้าภาพเกิน 16 ล้านพิกเซลจริง ๆ จะย่อ **พร้อมแจ้งผู้ใช้บนหน้าจอ** ไม่เงียบอีก | 🎨 เพิ่มเมนู แปลงภาพเป็นเวกเตอร์ ในหน้าออกแบบป้าย (โมดูลแยก vectora_api / vectora_engine / vectora_export · app.py เพิ่มแค่ include_router 1 บรรทัดในกรอบ try) — ขอบระดับย่อยพิกเซลจากสนามความเป็นสีต่อเนื่อง · ตัดสีขอบผสมทิ้งอัตโนมัติ (โลโก้ 5 สีเคยได้ขอบเทาปลอมเป็นสีที่ 6) · เดาจำนวนสีจากข้อศอกกราฟ RMS ΔE · ออกไฟล์ SVG/PDF/EPS/PNG/DXF (DXF เป็น SPLINE โค้งจริง 490KB→77KB และกลับแกน Y ให้ถูกทางเครื่องตัด) · วัดจริง: โลโก้ 250px 0.28 วิ · ภาพ 4000px 1.6 วิ · เส้นขาวแทรกระหว่างสี 0 พิกเซล · ไม่แตะโค้ดเส้นตัด/นีออนเดิมแม้แต่บรรทัดเดียว | 🔦 เส้นเดินไฟนีออนเส้นเดี่ยว: เปลี่ยนมาหาแกนกลาง 'ทีละตัวอักษร ก่อนรวม' — โลโก้ลายมือตัวอักษรซ้อนกันเป็นปกติ ถ้ารวมก่อนแล้วหาแกน ตรงที่ซ้อนจะกลายเป็นก้อนอ้วน แกนกลางแตกแขนงเป็นใยแมงมุม เส้นไฟเลยมั่ว · วัดจริง (Coffee 110 ซม.): รวมก่อน 22 ท่อน/ปมแตกแขนง 36 → ทีละตัว 11 ท่อน/ปม 15 · ท่อ 8 มม. เต็มลายเส้น 56.2% ล้นออกนอกตัวอักษร 0.85% · ความเร็ว 95.6 วิ → 0.7 วิ (คิดความละเอียดครั้งเดียวจากงานทั้งใบ ไม่ดันทีละตัวเป็น 3000px) · มีด่านตรวจก่อนใช้: ชิ้นที่เก็บไว้ต้องทับ full จริง ≥94% ไม่งั้นถอยไปวิธีเดิม · ไม่แตะ neon_route.py และเส้นคู่แม้แต่บรรทัดเดียว",
...
def _wrap_fit_outer(full, rec, trim_width_cm, trim_dir):
    ...
    if g <= 0.05 or full is None or full.is_empty:
        return full
    # ══════════════════════════════════════════════════════════════════
    # 🔧 วิธีย่อทรง: "สเกล" ไม่ใช่ "กัดขอบเข้า (erosion)"
    #
    # ⚠️ ผู้ใช้เจอจริง (2026-08-09 · งาน YN smile) จากการใช้ erosion:
    #      1) เส้นคิ้ว 'เบี้ยว' — กัดขอบเข้าแล้วขยายกลับด้วยมุม mitre
    #         ที่ร่องเว้าลึก (คอฟัน) กับปลายแหลม จะเกิดเดือย/เส้นไขว้ ขอบเลยหยัก
    #      2) ขนาดไม่ตรง — กัดเข้า d แล้วขยายออก d ไม่ใช่การผกผันกัน
    #         บางด้านคืนไม่ครบ (ได้ 225.8 × 86.6 ทั้งที่สั่ง 230 × 85)
    #      3) งานพิมพ์ไม่เต็มกล่องบนหน้าจำลองผนัง — เพราะทรงกล่องไม่ใช่งานย่อส่วน
    #         ลายพิมพ์จึงทาบไม่พอดี เหลือขอบว่างรอบ ๆ
    # ✅ สเกลแทน: ทรงกล่อง = 'งานย่อส่วน' เป๊ะ ๆ
    #      · ไม่มีเดือย ไม่มีเส้นไขว้ — ขอบเรียบเหมือนงานต้นฉบับทุกเส้น
    #      · ขนาดคำนวณตรง ๆ: ย่อให้เหลือ (เป้า − คิ้ว 2 ข้าง) พอใส่คิ้วกลับ = เป้าเป๊ะ
    #      · ลายพิมพ์ทาบเต็มหน้ากล่องพอดี (เพราะเป็นรูปเดียวกัน แค่คนละสเกล)
    #    สเกล 2 แกนแยกกันเล็กน้อย (~1-2%) เพราะคิ้วบวกเป็นค่าคงที่ ไม่ใช่สัดส่วน — ตาไม่เห็น
    # ══════════════════════════════════════════════════════════════════
    try:
        from shapely.affinity import scale as _sc, translate as _tr
        b = full.bounds
        W = b[2] - b[0]; H = b[3] - b[1]              # 🎯 เป้า = กรอบงานจริง = ขนาดที่ผู้ใช้กรอก
        if W <= 2.0 * g + 1.0 or H <= 2.0 * g + 1.0:
            return full                                # กล่องเล็กกว่าคิ้ว 2 ข้าง -> ไม่หัก (กันทรงหาย)
        # 🔁 ปรับสเกลจนขนาดนอกสุด (รวมคิ้ว) ตรงเป้า
        #    ที่ 'ปลายแหลม' เช่นปลายรากฟัน มุม mitre ดันออกเกินความกว้างคิ้ว (วัดได้ +1.1 ซม.)
        #    จึงต้องวัดของจริงแล้วปรับซ้ำ — ปรับได้ทั้งขยายและย่อ ลู่เข้าใน 2-3 รอบ
        fx = (W - 2.0 * g) / W
        fy = (H - 2.0 * g) / H
        best = None
        for _ in range(5):
            s = _sc(full, xfact=fx, yfact=fy, origin=(b[0], b[1]))
            if s is None or s.is_empty:
                break
            best = s
            o = s.buffer(g, join_style=2, mitre_limit=4.0, resolution=12)   # คิ้วจริงที่จะงอกออกมา
            if o is None or o.is_empty:
                break
            ob = o.bounds
            ow = ob[2] - ob[0]; oh = ob[3] - ob[1]
            if ow <= 0.01 or oh <= 0.01:
                break
            if abs(ow - W) <= 0.2 and abs(oh - H) <= 0.2:
                break                                   # ตรงเป้าภายใน 0.2 มม. -> พอ
            fx *= W / ow
            fy *= H / oh
        if best is None or best.is_empty:
            return full
        bb = best.bounds                                # จัดให้อยู่กลางกรอบเป้าหมาย
        s = _tr(best, xoff=b[0] + (W - (bb[2] - bb[0])) / 2.0 - bb[0],
                      yoff=b[1] + (H - (bb[3] - bb[1])) / 2.0 - bb[1])
        return s if (s is not None and not s.is_empty) else full
    except Exception:
        return full
...
def _text_full_from_spec(ts_spec, real_width_mm):
    ...
    try:
        ...
        for _ln in rows:
            ...
            if _b[3] - _b[1] <= 1e-6:
                return None
            # 📐 วางให้ 'เต็มกล่อง' ทั้งสองแกนแยกกัน
            #    ปกติหน้าเว็บส่งกล่องมาตามสัดส่วนฟอนต์อยู่แล้ว -> สเกล x กับ y เท่ากัน (ผลเหมือนเดิมเป๊ะ)
            #    แต่ถ้าผู้ใช้ 'ยืดกว้าง/ยืดสูง' บรรทัดนั้น กล่องจะไม่ได้สัดส่วนเดิม
            #    ถ้ายังสเกลเท่ากันสองแกน ไฟล์ตัดจะไม่ยืดตามที่เห็นบนจอ
            _sxf = (_x1 - _x0) / max(1e-6, _b[2] - _b[0])
            _syf = (_y1 - _y0) / max(1e-6, _b[3] - _b[1])
            _g = _sc9(_g, xfact=_sxf, yfact=_syf, origin=(_b[0], _b[1]))
            _b2 = _g.bounds
        ...
        if _file_subs and _file_ref:
            try:
                ...
                if _fu9 is not None and not _fu9.is_empty:
                    ...
                    #      _file_ref = กรอบของ pc["poly"] เฉพาะชิ้นที่ area > 4 ตร.มม.
                    #      _gb9      = กรอบของรูปที่ประกอบจาก 'เส้น (subs)' ของชิ้นเดียวกัน
                    #    ไฟล์ที่ poly กับ subs กินพื้นที่ไม่เท่ากัน (เช่นโลโก้ที่มีเส้น/clip กว้างกว่าเนื้อ)
                    #    จะได้อัตราส่วนเพี้ยน แล้วเส้นตัดถูกขยายทั้งชุด
                    #    (เคสจริง: 1800.0 / 1170.78 = 1.5374 -> แผ่นหลังกลายเป็น 276.7 ซม. แทน 180.0 ซม.)
                    # ✅ ความจริงคือ: _fu9 สร้างมาจาก subs ชุดเดียวกันนี้ -> อยู่ในระบบพิกัดเดียวกันอยู่แล้ว
                    #    กรอบอ้างอิงที่ถูกต้องจึงเป็น _gb9 เอง (แปลงแล้วได้เท่าเดิม = ไม่ต้องขยับ)
                    #    ไฟล์ที่สองกรอบตรงกันอยู่แล้ว ผลลัพธ์เท่าเดิมเป๊ะ (อัตราส่วน = 1.0 เหมือนเดิม)
                    _file_ref = _gb9
...
def _offset_subs_like_button(off_mm):
    ...
    try:
        ...
        # ขยายภาพก่อนทำ dilate/erode — ความละเอียดต้องผูกกับ 'ขนาดจริงของป้าย' ไม่ใช่ค่าคงที่
        # 🐞 บั๊กเส้นคิ้วโย้ (พบ 4 ส.ค. 69): เดิมตรึงไว้ 3000px เสมอ
        #    ป้ายกว้าง 322 ซม. -> ได้แค่ 0.93 พิกเซล/มม. -> คิ้ว 10 มม. = รัศมี 9 พิกเซล
        #    dilate ด้วยวงรีหยาบขนาดนั้นแล้วเทรซกลับ = ขอบเป็นคลื่น ไม่ขนานกับตัวอักษร
        #    (ป้ายเล็กไม่เจอ เพราะ 3000px ให้ 5 พิกเซล/มม. ขึ้นไป — จึงเพิ่งโผล่ตอนทำป้ายใหญ่)
        # ✅ ตั้งเป้า ≥ 4 พิกเซล/มม. เสมอ (เพดาน 12000px กันแรมบาน)
        _L = max(_g.shape[:2])
        _TG = max(3000.0, min(12000.0, float(_wmm) * 4.0))
        if abs(_L - _TG) > 1:
            _s = _TG / float(_L)
            _g = _cv9.resize(_g, None, fx=_s, fy=_s,
                             interpolation=(_cv9.INTER_AREA if _s < 1.0 else _cv9.INTER_CUBIC))
        ...
        # 🚧 ละเอียดไม่พอ = ขอบจะเป็นคลื่นแน่นอน -> คืน None ให้ผู้เรียกถอยไปใช้ offset แบบเรขาคณิต
        #    (ทางเรขาคณิตแม่นยำเป๊ะอยู่แล้ว — ชั้นแผ่นพื้น/อะคริลิคใช้ทางนั้นและเส้นตรงเป๊ะมาตลอด)
        if _ppm < 2.0:
            return None
...
def _offset_subs_from_geom(geom, off_mm, px_per_mm=5.0):
    ...
    try:
        ...
        # 🔎 ความละเอียดต้องพอกับ 'ระยะ offset' ด้วย ไม่ใช่แค่ขนาดชิ้น
        #    คิ้ว 8-10 มม. ถ้าได้รัศมีน้อยกว่า ~20 พิกเซล ขอบจะเป็นคลื่นทันที
        _ppm = float(px_per_mm)
        _need = 20.0 / max(1.0, abs(float(off_mm)))       # px/มม. ที่ทำให้รัศมี ≥ 20 px
        _ppm = max(_ppm, min(12.0, _need))
        _ppm = min(_ppm, 6000.0 / max(_w8, _h8))          # เพดานด้านยาว 6000 px (เดิม 3000)
        if _ppm < 1.5:
            return None
...
# ⏱️ งบเวลา/จำนวนครั้งของ 'เส้นตัดคมกริบ (ขยายที่ภาพ + potrace)'
#    🐞 บั๊กคิ้วเบี้ยว: งบ 8 วิ · 48 ครั้ง น้อยเกินไปสำหรับงาน 'ตัดแยกทีละตัว'
#       ป้าย COFFEE BEANS HOUSE = 64 ชิ้น -> ทำทันแค่ 24 ชิ้น อีก 31 ชิ้นหมดเวลา
#       ชิ้นที่หมดเวลาถอยไปใช้ buffer+ฟิตโค้ง -> คิ้วในไฟล์เดียวกันมี 2 มาตรฐานปนกัน
#       ตาจึงเห็นว่า 'บางตัวตรง บางตัวเบี้ยว' (กล่องไฟสี่เหลี่ยมตรงเป๊ะเพราะไม่ผ่านทางนี้เลย)
#    ✅ ขยายงบให้พอทำครบทุกชิ้นจริง (วัดแล้ว: 64 ชิ้น ใช้ ~12 วิ)
_SHARPSTAT = {"ok": 0, "reject": 0, "skip": 0, "t0": 0.0, "budget": 30.0,
              "calls": 0, "max_calls": 400}
...
def _sharp_offset(seed_geom, off_mm, target_geom):
    ...
    try:
        ...
        if float(_np2.percentile(_d, 98)) > max(0.6, min(_bw, _bh) * 0.006):
            ...
            return None
        # 3) 🆕 ด่าน 'ความเบี้ยวเฉพาะจุด' — จุดบอดของข้อ 2
        #    เปอร์เซ็นไทล์ 98 ยกเว้นจุดที่หลุดมากสุด 2% ทิ้งเสมอ (บนจุดตรวจ 400 จุด = ยกเว้นได้ 8 จุด)
        #    ความเบี้ยวที่กินพื้นที่แคบ ๆ เช่นหูแกะ จึงตกไปอยู่ใน 2% ที่ถูกยกเว้นพอดี แล้วผ่านด่านไปได้
        #    -> เติมเพดาน 'จุดใดจุดหนึ่งห้ามหลุดเกิน' ปิดช่องนั้น
        #    ⚠️ เพดานต้องผูกกับ 'ระยะขยาย/หด' ไม่ใช่เลขตายตัว เพราะมุมของสองวิธีต่างกัน
        #       ตามสัดส่วนของระยะ offset (ขยายที่ภาพ = มุมมน · buffer = มุมแหลม)
        #       ตั้งเลขตายตัว 3 มม. จะตีกลับเส้นที่ดีอยู่แล้วของชั้นคิ้วหนา ๆ ทันที
        #    📊 วัดจากชุดพิสูจน์ 84 เคส (โลโก้ After You ของจริง · 21 ชิ้น x ค่าเผื่อ 4 ค่า):
        #       ชิ้นที่ผ่านด่านอยู่แล้วทุกชิ้นอยู่ที่ 60% ของเพดานนี้เป็นอย่างมาก -> ไม่มีชิ้นดีชิ้นไหนถูกตีกลับ
        #    ⏱️ ใช้อาเรย์ _d ชุดเดิมที่คำนวณไปแล้ว จึงไม่มีต้นทุนเวลาเพิ่มแม้แต่นิดเดียว
        if float(_d.max()) > max(4.0, abs(float(off_mm)) * 4.0):
            _SHARPSTAT["reject"] = _SHARPSTAT.get("reject", 0) + 1
            return None
...
def _spot_strip_svg(spots, x, y, w, fs):
    """🎨 แถบ 'ช่องสีตัวอย่าง' แบบแคตตาล็อกสี — สี่เหลี่ยมชิ้นเล็ก + เบอร์ Pantone + การใช้งาน
       คืน (ชิ้นส่วน svg, ความสูงที่ใช้)"""
    if not spots:
        return [], 0.0
    p = []
    colw = (w - fs * 1.6) / max(1, len(spots))
    sw = min(fs * 3.4, colw * 0.30)                      # ด้านของช่องสี
    p.append('<text x="%.1f" y="%.1f" font-family="Prompt,Arial" font-size="%.1f" font-weight="800" '
             'fill="#0f172a">&#127912; สีที่ใช้ผลิต (เทียบเบอร์ Pantone)</text>' % (x, y + fs, fs * 1.02))
    ty = y + fs * 2.5
    for i, s in enumerate(spots):
        cx = x + i * colw
        p.append('<rect x="%.1f" y="%.1f" width="%.1f" height="%.1f" rx="%.1f" fill="%s" '
                 'stroke="#334155" stroke-width="%.2f"/>'
                 % (cx, ty, sw, sw, fs * 0.18, s["hex"], max(0.5, fs * 0.06)))
        tx = cx + sw + fs * 0.55
        p.append('<text x="%.1f" y="%.1f" font-family="Prompt,Arial" font-size="%.1f" font-weight="800" '
                 'fill="#0f172a">%s</text>' % (tx, ty + fs * 0.95, fs * 0.95, s["pantone"]))
        _sub = "%s · %s" % (s["hex"], s["name_th"])
        if s.get("toa"):
            _sub += " · TOA %s" % s["toa"]
        p.append('<text x="%.1f" y="%.1f" font-family="Prompt,Arial" font-size="%.1f" '
                 'fill="#475569">%s</text>' % (tx, ty + fs * 2.05, fs * 0.78, _sub))
        if s.get("use"):
            p.append('<text x="%.1f" y="%.1f" font-family="Prompt,Arial" font-size="%.1f" '
                     'fill="#64748b">%s</text>'
                     % (tx, ty + fs * 3.05, fs * 0.74,
                        str(s["use"]).replace("&", "&amp;").replace("<", "&lt;")))
        if not s.get("exact") and float(s.get("delta_e") or 0) > 5.0:
            p.append('<text x="%.1f" y="%.1f" font-family="Prompt,Arial" font-size="%.1f" '
                     'fill="#b45309">เทียบใกล้เคียง &#916;E %.1f</text>'
                     % (tx, ty + fs * 3.95, fs * 0.72, float(s["delta_e"])))
...
def _spot_box_svg(spots, bx, by, bw, fs_in=0.0, ncol=0):
    ...
    p = ['<g id="spotbox">',
         ...
         % (bx, by, bw, bh, fs * 0.6, max(0.6, fs * 0.08)),
...
def _weight_panel_svg(svg, wsum, tube=None, chk=None, span_mm=0.0, fits=True, extra_lines=None,
                      boq_items=None, spots=None):
    """⚖️🧾🎨 ต่อ 'แผงข้อมูล' ไว้ใต้ภาพ 3 มิติ — น้ำหนัก · โครงสร้างเหล็ก · BOQ ติดตั้ง · สีที่ใช้ผลิต

    ⚠️ บทเรียน 2 ข้อจากการใช้จริง:
      1) เคยวางทับบนภาพ -> บังงาน มองแบบไม่เห็น
      2) เคยเรียงยาวเป็นแถวเดียว 40 บรรทัด -> ภาพสูงขึ้นเท่าตัว ต้องเลื่อนจอหาแบบ
    ✅ ตอนนี้จัดเป็น 3 คอลัมน์เคียงกัน (กว้างแต่เตี้ย) แล้วต่อแถบสีไว้ล่างสุด
       ตัวภาพ 3 มิติไม่ถูกแตะเลยแม้แต่จุดเดียว — แค่ขยายกรอบลงล่างแล้ววาดเพิ่ม
    """
    try:
        import re as _re9
        ...
        # ── คอลัมน์ 1: น้ำหนัก ──────────────────────────────────────────
        c1 = ["⚖️ น้ำหนักป้ายโดยประมาณ  %.1f กก." % float(wsum.get("total_kg") or 0.0)]
        _d = []
        for k, lb in (("sheet_kg", "แผ่นวัสดุ"), ("led_kg", "ไฟ+หม้อแปลง"), ("frame_kg", "โครงเหล็ก")):
            if float(wsum.get(k) or 0) > 0:
        ...
        c2 = []
        if tube:
            c2.append("🔩 เหล็กกล่อง %s หนา %.1f มม. · ช่วงพาด %.0f ซม."
                      % (tube.get("label", ""), float(tube.get("t") or 0), float(span_mm) / 10.0))
            if chk:
                c2.append("   หน่วยแรง %.0f MPa (ไม่เกิน 140) · แอ่นตัว %.1f มม. (ไม่เกิน %.1f)"
                          % (chk.get("sigma", 0), chk.get("defl", 0), chk.get("defl_lim", 0)))
            c2.append("   ✅ รับน้ำหนักได้ปลอดภัย" if fits
                      else "   ⚠️ ช่วงพาดยาวเกิน — ต้องเพิ่มจุดยึด ไม่ใช่เพิ่มขนาดเหล็ก")
        for _l in (extra_lines or []):
            c2.append("   " + str(_l))
        c2.append("   * เป็นน้ำหนักตัวป้าย ยังไม่รวมแรงลม (ป้ายกลางแจ้งต้องคิดเพิ่ม)")

        # ── คอลัมน์ 3: BOQ ของสิ้นเปลืองติดตั้ง ──────────────────────────
        c3 = []
        if boq_items:
        ...
        if rec.get("wrap"):
            ...
            try:
                if _tight9:
                    ...
                    #
                    # ⚠️ ผู้ใช้เจอจริง (2026-08-09): เบอร์โทรกับตัวโลโก้หลุดลงมาอยู่ 'นอกกล่อง'
                    #    ต้นเหตุ: สูตรด้านล่างเก็บสัดส่วน 'กรอบงานจริง ÷ กรอบทรงกล่อง'
                    #    พอโหมดนี้หักความกว้างคิ้วออก ตัวหารเล็กลง -> สัดส่วนเกิน 1
                    #    รูปจึงถูกวาดใหญ่กว่าหน้ากล่องแล้วเลื่อนหลุดออกไป
                    #    (โหมดเดิมกล่องพองใหญ่กว่างานมาก สัดส่วน < 1 เสมอ จึงไม่เคยเห็นอาการนี้)
                    _ART_B0 = (0.0, 0.0, 1.0, 1.0)
                else:
                    # 🖨️ สูตรเดิม (ทางเดินเดิม ห้ามแตะ): ทาบงานพิมพ์ตามตำแหน่งจริงเทียบกรอบทรงกล่อง
                    _qb0 = full.bounds
        ...
        for _btag, _brec, _bfull in _builds:
          ...
          for L in ([] if _neon else rec["layers"]):
              ...
              if _use_raw_punch:
                  ...
                  # ══════════════════════════════════════════════════════════
                  # ⚠️ ผู้ใช้เจอจริง (2026-08-09 · งาน YN smile): 'เส้นตัดขอบคิ้วเบี้ยว'
                  #    ขอบในกับขอบนอกของคิ้วสร้างมาขนานกันเป๊ะ (o2 = full+band · i2 = full)
                  #    แต่ตอนแปลงเป็นเส้นตัด ชั้นที่ clean=True จะถูก 'ฟิตเป็นเส้นโค้ง'
                  #    -> เส้นตรงกลายเป็นโค้งอ่อน ๆ ขอบโย้ (เคยวัดได้ 7.6-12.9 มม.)
                  #       และโย้คนละทางกันระหว่างขอบใน/ขอบนอก จึงเห็นเป็น 'สอบเข้าหากัน'
                  # ✅ โหมด 'โลโก้เต็มพื้นที่กล่อง' (tight) = ทรงมาจาก union ของเวกเตอร์จริง
                  #    สะอาดอยู่แล้ว ไม่มีคลื่นพิกเซลให้รีด -> ห้ามฟิตโค้ง เหมือนกล่องทรงเรขาคณิต
                  #    (งานที่ไล่เส้นจากภาพ ยังฟิตโค้งเหมือนเดิมทุกประการ)
                  # ══════════════════════════════════════════════════════════
                  _isgeo = ((bool(rec.get("box_shape"))
                             or bool(rec.get("wrap") and str(wrap_tight) == "1"))
                            and kind in ("solid", "frame", "backing"))
...
/* 📸 โหมดแคปภาพ (JPG/PDF): html2canvas วาดคอลัมน์แบบ column-count ไม่ได้ -> การ์ดกลายเป็นกล่องเปล่า
   ระหว่างแคปจึงสลับเป็นกริดธรรมดาชั่วคราว แล้วคืนสภาพทันทีหลังแคปเสร็จ */
.sheet.capmode .masonry{column-count:auto;display:grid;grid-template-columns:repeat(5,1fr);gap:7px;align-items:start}
.sheet.capmode .masonry .card{display:block;width:auto;margin:0}
...
<!-- ⚡ ไม่โหลดสคริปต์แคปภาพตั้งแต่เปิดหน้า — โหลดเฉพาะตอนกด 'บันทึก JPG' เท่านั้น
     (ใบสั่งผลิตเปิดบ่อยกว่าการแคปภาพมาก · เดิมโหลด 2 ไลบรารีทุกครั้งจนหน้าอืด)
     ส่วน PDF ใช้ตัวพิมพ์ของเบราว์เซอร์เองแล้ว = คมชัดกว่า เร็วกว่า และไฟล์เล็กกว่ามาก -->
<script>
var JOBNO="__JOBNO__";
...
function _cap(cb){_busy(true);
  var _sh0=document.querySelector('.sheet');
  if(_sh0)_sh0.classList.add('capmode');             // 📸 สลับเป็นกริดก่อนแคป (กันการ์ดกลายเป็นกล่องเปล่า)
  var _done=function(){ if(_sh0)_sh0.classList.remove('capmode'); _fitForCapture(false); _busy(false); };
  var fr=(document.fonts&&document.fonts.ready)?document.fonts.ready:Promise.resolve();
  fr.then(function(){return new Promise(function(r){setTimeout(r,260);});}).then(function(){
    var _sh=document.querySelector('.sheet');
    var _W=_sh.scrollWidth||1900;                    // ⚠️ ต้องอ่านความกว้างจริง ห้ามฝังเลขตายตัว
    var _f=_fitForCapture(true);                     // 📄 A3 แนวนอน 1 แผ่นเสมอ
    // ⚡ scale 1.4 ก็คมพอสำหรับ A3 แล้ว (เดิม 2 = พื้นที่วาดมากกว่าเท่าตัว ช้าโดยไม่จำเป็น)
    var _opt={scale:1.4,backgroundColor:'#ffffff',useCORS:true,logging:false};
    if(_f<0.999){ _opt.width=Math.ceil(_W*_f); _opt.height=Math.ceil((_sh.scrollHeight||1)*_f);
                  _opt.windowWidth=_W; }
    setTimeout(function(){
      html2canvas(_sh,_opt)
        .then(function(cv){_done();cb(cv);})
        .catch(function(e){_done();alert('สร้างภาพไม่ได้: '+e);});
    },120);
  });}
function saveJPG(){
  // ⚡ โหลด html2canvas ตอนใช้จริงเท่านั้น
  var go=function(){_cap(function(cv){var a=document.createElement('a');
    a.href=cv.toDataURL('image/jpeg',.9);a.download='JobSheet_'+JOBNO+'.jpg';a.click();});};
  if(window.html2canvas){go();return;}
  _busy(true);
  var sc=document.createElement('script');
  sc.src='https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js';
  sc.onload=function(){_busy(false);go();};
  sc.onerror=function(){_busy(false);alert('โหลดตัวแคปภาพไม่ได้ — ใช้ปุ่ม "พิมพ์" แล้วเลือก Save as PDF แทนได้ค่ะ');};
  document.head.appendChild(sc);
}
// 📄 PDF = ใช้ตัวพิมพ์ของเบราว์เซอร์ ไม่ใช่แปลงเป็นภาพแล้วยัดใส่ PDF
//    ข้อดีที่วัดได้: เร็วขึ้นมาก (ไม่ต้องวาดแคนวาสหลายพันพิกเซล) · ตัวหนังสือคมเป็นเวกเตอร์จริง
//    · ไฟล์เล็กลงหลายเท่า · และไม่เพี้ยนเพราะ html2canvas วาด CSS บางอย่างไม่ได้
function savePDF(){ try{_fitOnePage();}catch(e){} setTimeout(function(){window.print();},60); }
</script>
</body></html>'''
...
def _serve_app(request: Request, path=None):
    ...
    # 🔄 หน้าเว็บต้องถามเซิร์ฟเวอร์ทุกครั้งว่า "มีของใหม่ไหม" (ยังใช้ ETag/304 อยู่ ไม่เปลืองเน็ต)
    #    ถ้าไม่ใส่ เบราว์เซอร์จะเดาเองว่าไฟล์ยังใหม่อยู่แล้วใช้ของเก่าในเครื่องต่อ
    #    -> deploy แล้วผู้ใช้ยังเห็นหน้าเดิม ทั้งที่หลังบ้านอัปเดตไปแล้ว (เจอมาแล้วจริง)
    try:
...
async def extract_assets(file: UploadFile = File(...), split_gap: float = Form(0.0)):
    ...
    try:
        ...
        low = name.lower()
        if not low.endswith((".pdf", ".ai", ".eps", ".ps", ".svg")):
            return JSONResponse({"error": "รองรับเฉพาะ SVG / PDF / .ai / .eps"}, status_code=400)
        # 🆕 .svg -> แปลงเป็น PDF ก่อน แล้วเข้าสายงานเดิมทั้งเส้น
        #    ⚠️ เดิม .svg ถูกโยนไปเส้นทาง 'ภาพ' ทำให้ทั้งโลโก้กลายเป็นชิ้นเดียว แตกชิ้นไม่ได้เลย
        #       (เคสจริง YN smile: ในไฟล์มี 58 รูปทรง แต่หน้าจอขึ้นว่า "1 ชิ้น · ภาพ")
        #    แปลงผ่าน cairosvg จัดการ transform/group/mask ให้ครบ พิกัดกับสีจึงตรงตามที่ตาเห็น
        if low.endswith(".svg"):
            import vectora_vector as _vv
            with open(inp, "rb") as _f:
                _raw = _f.read()
            _pdf = os.path.join(tmp, "from_svg.pdf")
            with open(_pdf, "wb") as _f:
                _f.write(_vv._to_pdf(_raw, name))
            inp = _pdf
    ...
    try:
        ...
        for it in items:
            try:
                ...
                # ══════════════════════════════════════════════════════════════
                # ⚠️ ผู้ใช้เจอจริง (2026-08-09 · งาน YN smile): รูปฟันสีขาวหายจากเส้นตัดทั้งใบ
                #    ต้นเหตุ: หน้าเว็บแนบ png มากับ 'ทุกชิ้นที่ไม่ใช่พื้นหลัง' แม้ชิ้นนั้นมี token เวกเตอร์
                #    โค้ดตรงนี้เห็น png ก็ฝังเป็นภาพแล้ว continue ข้าม token ทิ้งไปเลย
                #    -> composed_vector.pdf กลายเป็น 'ภาพล้วน' ไม่มี path สักเส้น
                #    -> ขั้นทำเส้นตัดต้องถอยไปไล่เส้นจากภาพด้วย potrace
                #    -> ชิ้นสีขาวบนพื้นขาว (ตัวฟัน) ถูกนับเป็นพื้นหลัง หายเกลี้ยง
                # ✅ ให้ 'เส้นเวกเตอร์ในไฟล์' มาก่อนเสมอ — มี token เมื่อไหร่ใช้ token
                #    (หน้าเว็บรุ่นใหม่จะไม่ส่ง png มาให้อยู่แล้วเมื่อเปิดโหมดเวกเตอร์
                #     เงื่อนไขนี้เป็นเกราะกันหน้าเว็บรุ่นเก่าที่ยังส่งมาทั้งคู่)
                # ══════════════════════════════════════════════════════════════
                if it.get("png") and not it.get("token"):
...
# ==================================================================
#  ✍️🎨 สร้าง "โลโก้แบรนด์" ให้จบในหน้าแตกชิ้นเดิม — ข้อความ + พื้นหลัง
# ==================================================================
#  ผู้ใช้ขอ (2026-08-09): "ในหน้านี้ ให้มองว่าเป็นการสร้าง logo brand ก่อนจะนำไปวางกับ
#  ประเภทป้าย ... ขอให้มีการเพิ่ม font text ข้อความพิมพ์ได้อิสระ ปรับรูปแบบได้ด้วย
#  รวมถึงการสร้างพื้นหลังให้จบจากตรงนี้ เพื่อโยนเข้าไปสร้างประเภทป้ายต่อได้"
#
#  🔑 หัวใจของการออกแบบ: **ไม่สร้างหน้าจอใหม่**
#     สองปุ่มนี้คืนของออกมาเป็น "ชิ้นงาน" หน้าตาเหมือนชิ้นที่แตกจากไฟล์ลูกค้าเป๊ะ
#     (token + page + bbox + w_mm/h_mm + รูปพรีวิว) สายงานเดิมทั้งเส้นจึงใช้ได้ทันที
...
def text_fonts():
    """รายชื่อฟอนต์ที่ 'มีไฟล์อยู่ในเครื่องเราจริง' + บอกว่าตัวไหนพิมพ์ไทยได้

    ⚠️ ห้ามใช้รายชื่อฟอนต์ของ Google CDN ที่หน้าเว็บใช้แสดงผลบนจอ —
...
@app.post("/api/text-asset")
async def text_asset(request: Request):
    """✍️ พิมพ์ข้อความ -> ชิ้นงานเวกเตอร์แท้ (เส้นโค้งจากไฟล์ฟอนต์ ไม่ผ่านภาพเลยสักขั้น)
    ...
    try:
        ...
        if not txt.strip():
            return JSONResponse({"ok": False, "error": "ยังไม่ได้พิมพ์ข้อความ"}, status_code=400)
        if len(txt) > 400:
            txt = txt[:400]
        ...
        svg, _mmu = _tv_text_svg(
            ...
            tracking=max(-0.3, min(1.5, float(b.get("tracking") or 0.0))),
            line_gap=max(0.55, min(3.0, float(b.get("line_gap") or 1.0))),
            align=str(b.get("align") or "center"),
        ...
        return JSONResponse({"ok": False, "error": str(e),
                             "trace": traceback.format_exc()[-400:]}, status_code=400)


@app.post("/api/bg-asset")
async def bg_asset(w_cm: float = Form(20.0), h_cm: float = Form(10.0),
                   shape: str = Form("rect"), radius_cm: float = Form(0.0),
                   fill: str = Form("solid"),
                   color1: str = Form("#2563eb"), color2: str = Form("#7c3aed"),
                   angle: float = Form(90.0), art_fit: str = Form("cover"),
                   border_cm: float = Form(0.0), border_color: str = Form("#0f172a"),
    ...
    try:
        ...
        if fill == "art":
            if file is None:
                return JSONResponse({"ok": False, "error": "เลือกไฟล์ artwork ก่อนนะคะ"}, status_code=400)
            art = await file.read()
            ...
            if nm.endswith((".jpg", ".jpeg")):
                ...
                mime = "image/webp"
            elif nm.endswith(".svg"):
                # SVG เป็นเวกเตอร์อยู่แล้ว -> แปลงเป็น PNG ความละเอียดสูงก่อนฝัง
                # (ฝัง SVG ซ้อน SVG แล้ว cairosvg อ่านต่อไม่ได้)
                import cairosvg as _cs
                art = _cs.svg2png(bytestring=art, output_width=2400)
                mime = "image/png"
        ...
        svg = _tv_bg_svg(
            ...
            radius_mm=max(0.0, float(radius_cm or 0) * 10.0),
            fill=str(fill or "solid"), color1=color1, color2=color2, angle=float(angle or 0),
            art_bytes=art, art_mime=mime, art_fit=str(art_fit or "cover"),
...
# ══════════════════════════════════════════════════════════════════
# 🔒 ทำให้สถิติ "ไม่หาย" จริง ๆ — คิวส่งซ้ำ + รายงานสุขภาพ
#
# ⚠️ ความจริงของที่ตั้งตอนนี้ (ตรวจแล้ว 2026-08-09): Render แพลนฟรี ต่อดิสก์ถาวรไม่ได้
#    ไฟล์ .db และสมุด .jsonl จึงอยู่บน /tmp = **หายทุกครั้งที่ deploy และทุกครั้งที่เครื่องตื่นจากสลีป**
#    ที่ถาวรจริงมีที่เดียวคือ Google Sheet
# ⚠️ ช่องโหว่ที่ยังเหลือ: ยิงเข้าชีตเป็นแบบ 'ยิงแล้วลืม' ถ้าพลาด (เน็ตสะดุด · Apps Script ตื่นช้า ·
#    เครื่องสลีปตัดเธรดทิ้งกลางคัน) event นั้น **หายถาวร** ไม่มีใครรู้ ไม่มีการลองใหม่
# ✅ แก้ด้วย 3 อย่าง:
#    1) พลาดเมื่อไหร่ -> เก็บเข้า 'คิวค้างส่ง' ทันที
#    2) ลองส่งซ้ำให้เองตอนบูต และทุกครั้งที่มี event ใหม่เข้ามา
#    3) รายงานให้เห็นชัดว่า 'ถาวรอยู่ไหม' + ค้างกี่รายการ + พลาดล่าสุดเพราะอะไร
#       (ของเดิมไม่มีทางรู้เลยว่าชีตพังไปตั้งแต่เมื่อไหร่)
# ══════════════════════════════════════════════════════════════════
_AN_PEND = (_AN_DB[:-3] if _AN_DB.endswith(".db") else _AN_DB) + ".pending.jsonl"
_AN_PLOCK = threading.Lock()
_AN_HEALTH = {"ok_n": 0, "fail_n": 0, "last_ok": "", "last_err": "", "last_try": ""}


def _an_pend_add(row):
    """ยิงเข้าชีตไม่สำเร็จ -> พักไว้ในคิว รอส่งใหม่ (ห้ามทิ้ง)"""
    try:
        with _AN_PLOCK:
            with open(_AN_PEND, "a", encoding="utf-8") as f:
                f.write(json.dumps(dict(zip(_AN_COLS, row)), ensure_ascii=False) + "\n")
        return True
    except Exception:
        return False
...
def _an_pend_rows():
    try:
        with open(_AN_PEND, encoding="utf-8") as f:
...
def _an_pend_count():
    return len(_an_pend_rows())


def _an_flush_pending(limit=200):
    """ลองส่งคิวค้างเข้าชีตใหม่ · ที่ส่งสำเร็จค่อยตัดออกจากคิว
       ⚠️ ต้องเขียนคิวใหม่ทั้งไฟล์แบบ 'เขียนไฟล์ชั่วคราวแล้วสลับ' กันไฟดับกลางคันแล้วคิวหาย"""
    lines = _an_pend_rows()
    if not lines:
        return 0
    left = []
    ...
    for i, line in enumerate(lines):
        if sent >= int(limit):
            left.append(line); continue
        try:
            d = json.loads(line)
            row = tuple(d.get(k, "") if k != "dur" else int(d.get("dur", 0) or 0)
                        for k in _AN_COLS)
        except Exception:
            continue                                   # บรรทัดเสีย -> ทิ้ง
        ok, _msg = _push_sheet(row, blocking=True, queue_on_fail=False)
        if ok:
            sent += 1
        else:
            left.append(line)
    try:
        with _AN_PLOCK:
            ...
            with open(tmp, "w", encoding="utf-8") as f:
                f.write("".join(l + "\n" for l in left))
            os.replace(tmp, _AN_PEND)
    except Exception:
        pass
    if sent:
        print("[analytics] ส่งคิวค้างสำเร็จ %d รายการ · เหลือ %d" % (sent, len(left)), flush=True)
    return sent
...
def _an_flush_bg():
    ...
    if not os.path.exists(_AN_PEND):
        return
    _an_flush_bg._busy = True

    def _go():
        try:
            _an_flush_pending()
        finally:
            _an_flush_bg._busy = False
    threading.Thread(target=_go, daemon=True).start()


def _an_health():
    """สรุปให้คนอ่านรู้เรื่องว่า 'สถิติถาวรอยู่ไหม' — ห้ามบอกกำกวม"""
    pend = _an_pend_count()
    hook = bool(_sheet_hook())
    # ⚠️ ลำดับการตัดสินสำคัญมาก — ถ้ามีดิสก์ถาวร ข้อมูล "ปลอดภัยแล้ว" ไม่ว่าชีตจะพังหรือไม่
    #    เคยเรียงผิด (เช็คคิวค้างก่อนเช็คดิสก์) แล้วขึ้นแดงว่า 'ยังไม่ถาวร' ทั้งที่ดิสก์ถาวรอยู่
    if _AN_PERSIST:
        if not hook:
            lv, note = "ok", ("✅ ถาวร — เก็บบนดิสก์ถาวร (%s) ไม่หายตอน deploy "
                              "(ยังไม่ได้ต่อ Google Sheet เป็นตัวสำรอง)" % _AN_DB)
        elif _AN_HEALTH["fail_n"] and not _AN_HEALTH["ok_n"]:
            lv, note = "warn", ("✅ ข้อมูลปลอดภัยบนดิสก์ถาวร (%s) แต่ยิงเข้า Google Sheet ไม่สำเร็จ (%s) "
                                "— ตัวสำรองยังไม่ทำงาน เก็บเข้าคิวไว้ %d รายการแล้ว"
                                % (_AN_DB, (_AN_HEALTH["last_err"][:70] or "ไม่ทราบสาเหตุ"), pend))
        elif pend:
            lv, note = "warn", ("✅ ถาวรบนดิสก์แล้ว · มีค้างส่งเข้าชีตอยู่ %d รายการ "
        ...
        lv, note = "ok", ("✅ ถาวรผ่าน Google Sheet — ดิสก์เป็นแบบชั่วคราว (%s) "
                          "แต่ข้อมูลจริงอยู่บนชีต ไม่หายตอน deploy" % _AN_DB)
    return {"level": lv, "note": note, "persist_disk": _AN_PERSIST, "db": _AN_DB,
            "sheet": hook, "pending": pend, "sent_ok": _AN_HEALTH["ok_n"],
            "sent_fail": _AN_HEALTH["fail_n"], "last_ok": _AN_HEALTH["last_ok"],
            "last_err": _AN_HEALTH["last_err"][:200], "log_rows": _an_log_rows()}
...
try:
    ...
    _AN_RESTORED = 0
# 📤 ลองส่ง 'คิวค้างส่ง' เข้า Google Sheet ตั้งแต่ตอนบูต
#    เคสจริงที่กันได้: เครื่องสลีป/รีสตาร์ตตอนกำลังยิง -> เธรดถูกตัดกลางคัน event นั้นเคยหายถาวร
try:
    threading.Thread(target=_an_flush_pending, daemon=True).start()
except Exception:
    pass
...
async def api_track(request: Request):
    ...
    # 👤 ถ้าหน้าเว็บส่งมาว่า guest แต่คนนี้ถือตั๋ว SSO อยู่ -> ใช้ชื่อจากตั๋วแทน
    #    ⚠️ เคสจริง (ผู้ใช้ทัก 2026-08-09): เปิดเว็บตรง ๆ ไม่ผ่าน CRM Hub
    #       หน้าเว็บไม่มีชื่อจะส่ง เลยส่ง guest มา สถิติจึงขึ้นว่า guest ทั้งที่เป็นเจ้าของเอง
    #    ✅ ฝั่งเซิร์ฟเวอร์รู้จากตั๋วอยู่แล้ว ก็เติมให้เลย ไม่ต้องพึ่งหน้าเว็บอย่างเดียว
    _acc = str(d.get("account", "") or "").strip()
    _nick = str(d.get("nick", "") or "").strip()
    _full = str(d.get("fullname", "") or "").strip()
    if (not _acc) or _acc.lower() == "guest":
        try:
            from vectorcnc import auth as _A
            _pl = _A.verify(_token_of(request)) or {}
        except Exception:
            _pl = {}
        # ⚠️ ตั๋วของระบบนี้เก็บอีเมลไว้ที่คีย์ "e" (ดู vectorcnc/auth.py: sign / sign_internal)
        #    เคยเดาว่าเป็น "u" แล้วอ่านไม่เจอ -> ยังขึ้น guest เหมือนเดิม
        _acc = (str(_pl.get("e", "") or _pl.get("u", "") or _pl.get("email", "")
                    or "").strip() or _acc)
        _nick = _nick or str(_pl.get("n", "") or _pl.get("nick", "") or "").strip()
        _full = _full or str(_pl.get("f", "") or _pl.get("name", "") or "").strip()
        if not _acc:
            _q = str(request.query_params.get("u", "") or "").strip()
            _acc = _q or "guest"
    row = (now.isoformat(timespec="seconds"), now.strftime("%Y-%m-%d"),
           str(d.get("sid", ""))[:40], _acc[:60],
           ...
           _nick[:60], _full[:80])                                        # 👤 ชื่อเล่น · ชื่อจริง
    ok_local = True
    try:
        with _AN_LOCK:
            c = _an_conn()
    ...
    _an_flush_bg()                         # ถือโอกาสเคลียร์คิวที่เคยส่งพลาดไปด้วย
    return {"ok": True, "local": ok_local, "log": ok_log,
            "pending": _an_pend_count()}
...
def _push_sheet(row, blocking=False, queue_on_fail=True):
    ...
    def _wrap():
        _AN_HEALTH["last_try"] = datetime.now(TZ7).isoformat(timespec="seconds")
        try:
            ok, msg = _go()
        except Exception as e:
        ...
        if ok:
            _AN_HEALTH["ok_n"] += 1
            _AN_HEALTH["last_ok"] = _AN_HEALTH["last_try"]
        else:
            _AN_HEALTH["fail_n"] += 1
            _AN_HEALTH["last_err"] = str(msg)
            # ⚠️ หัวใจของความถาวร: ยิงพลาดแล้วห้ามทิ้ง — พักไว้ในคิว รอส่งใหม่
            if queue_on_fail:
                _an_pend_add(row)
        return ok, msg
...
def api_stats_health(request: Request, test: int = 0):
    """🩺 ตรวจว่า 'สถิติถาวรจริงไหม' — 🔒 แอดมินเท่านั้น

    test=1 จะยิงแถวทดสอบเข้า Google Sheet แบบรอผล (ไม่ใช่ยิงแล้วลืม) แล้วรายงานสาเหตุถ้าพลาด
    ทำไมต้องมี: ของเดิมถ้าชีตล่ม จะไม่มีใครรู้เลยจนกว่าจะเปิดดูสถิติแล้วพบว่าตัวเลขไม่ขึ้น
    """
    if not (_is_internal(request) and _is_admin(request)):
        return JSONResponse({"ok": False, "error": "forbidden",
                             "msg": "เปิดให้เฉพาะผู้ดูแลระบบ"}, status_code=403)
    h = _an_health()
    out = {"ok": True, "health": h}
    if int(test or 0):
        now = datetime.now(TZ7)
        row = (now.isoformat(timespec="seconds"), now.strftime("%Y-%m-%d"),
               "HEALTHCHK", "system", "visit", "/", "ตรวจระบบสถิติ", "", "(ตรวจระบบ)",
               "server", "selftest", 0, "", "")
        ok, msg = _push_sheet(row, blocking=True, queue_on_fail=False)
        out["test"] = {"ok": bool(ok), "detail": str(msg)[:300],
                       "msg": ("✅ เขียนลง Google Sheet ได้จริง — สถิติถาวรแน่นอน" if ok else
                               "❌ เขียนลง Google Sheet ไม่ได้: " + str(msg)[:200])}
        out["health"] = _an_health()
    if h["pending"]:
        out["flushed"] = _an_flush_pending(limit=500)
        out["health"] = _an_health()
    return out
...
def api_stats(request: Request, days: int = 30, fresh: int = 0):
    ...
    if j:
        j["health"] = _an_health()          # 🔒 บอกให้ชัดทุกครั้งว่าสถิติถาวรอยู่ไหม
        return j
    try:
        with _AN_LOCK:
            c = _an_conn()
...
# ══════════════════════════════════════════════════════════════════════════
# 🎨 Vectora — เมนู "แปลงภาพเป็นเวกเตอร์" (โมดูลแยก · เพิ่มเข้ามา 2026-08-07)
#    โค้ดทั้งหมดอยู่ใน vectora_api.py / vectora_engine.py / vectora_export.py
#    ห่อ try ไว้เพื่อว่าถ้าโมดูลนี้มีปัญหา แอปเดิมต้องยังขึ้นได้ตามปกติ
# ══════════════════════════════════════════════════════════════════════════
try:
    from vectora_api import router as _vectora_router
