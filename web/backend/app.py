...
BUILD_TAG = "2026-08-09-stats-clear"
# 📌 สรุปอัปเดตล่าสุดแบบ "หนึ่งบรรทัด" สำหรับโชว์บนหัวเว็บ
#    (build_note ตัวเต็มยาวเป็นหน้ากระดาษ เอาไปขึ้นหัวเว็บไม่ได้)
BUILD_SHORT = "สถิติบอกสถานะถูกต้อง + จำชื่อผู้ใช้จากตั๋ว · ป้ายแพลนบนหัวเว็บ · ปิดปุ่มทดลองฟรีชั่วคราว"
...
async def api_track(request: Request):
    ...
    if (not _acc) or _acc.lower() == "guest":
        try:
            ...
            _pl = {}
        _acc = str(_pl.get("u", "") or _pl.get("email", "") or "").strip() or _acc
