...
UP_SNAP = 1                    # 1 = ปัด "ตัวคูณขยายภาพ" ให้เป็นจำนวนเต็ม (เฉพาะโหมดไล่สี)
# ══════════════════════════════════════════════════════════════════
# 🔎 ของจริงที่วัดได้ 2026-08-22 — ตัวแปรสำคัญไม่ใช่ "ความละเอียดสูงแค่ไหน"
#    แต่คือ "ตัวคูณขยายเป็นจำนวนเต็มหรือเปล่า"
#    ขยายด้วยจำนวนเต็ม พิกเซลใหม่ตกลงบนตารางเดิมพอดี — พิกเซลต้นฉบับ 1 ตัว
#    กลายเป็นบล็อก k×k เป๊ะ ๆ ขอบของบล็อกจึงตรงกับขอบจริงของภาพ
#    ขยายด้วยเศษ (เช่น 3.25 เท่า) ขอบของทุกพิกเซลไปตกคร่อมตารางใหม่คนละที่กัน
#    -> ตัวอักษรสูง 8 พิกเซลถูกเกลี่ยไม่เท่ากันในแต่ละตัว = ตัว M เละ · nai มีจุดส้มแทรก
#
#    วัดจริงกับป้าย 554 px (ตัวคูณ / ภาพทำงาน / เวลา / ค่าคลาดขอบ):
#      2.00x -> 1108 px ·  30 วิ · 7.98   ตัวอักษรเริ่มขาด (I ของ LINEMAN หาย)
#      3.25x -> 1800 px ·  64 วิ · 7.69   ← ของเดิม เศษ .25 · M เป็นก้อน · nai มีจุดส้ม
#      3.00x -> 1662 px ·  57 วิ · 7.68   เร็วกว่าเดิม 11% และ M/nai ดีกว่าเดิมชัด
#      4.00x -> 2216 px ·  97 วิ · 7.66   ดีที่สุด M คม nai สะอาด วงส้มไม่แหว่ง
#    -> ไฟล์เล็ก (โลโก้ที่ลูกค้าส่งมา) ปัดขึ้น · ไฟล์อื่นปัดหาจำนวนเต็มใกล้สุด (ไม่แพงขึ้น)
# ══════════════════════════════════════════════════════════════════
SMALL_SRC = 1100.0             # ต้นฉบับด้านยาว <= นี้ = "ไฟล์เล็ก" ปัดตัวคูณขึ้น
UP_HEAD = 1.35                 # ปัดขึ้นได้ไม่เกินกี่เท่าของเป้าหมายที่แรมเครื่องรับไหว
#  🛡️ กันเครื่องเล็กพัง: เครื่อง 512 MB เป้าหมาย 1200 px จะปัดขึ้นได้ไม่เกิน 1620 px
#     (ตัวคูณ 3 เท่าของไฟล์ 554 px = 1662 px เกินเพดาน -> ถอยกลับไปใช้ 2 เท่า)
TOL_SCALE = 1.0                # ตัวคูณเกณฑ์ความคลาดตอนฟิตเบซิเยร์ (เฉพาะโหมดไล่สี)
#  ⚠️ วัดแล้วผ่อนเกณฑ์นี้แทบไม่ช่วยเวลา (97 -> 85 วิ) และไม่ได้ทำให้ภาพดีขึ้น
#     ตัวที่ให้ผลจริงคือ UP_SNAP ด้านบน จึงคงไว้ที่ 1.0 (ไม่ผ่อน)
...
CLEAN_MERGE = False            # ⛔ ขั้น B (รวมชิ้นสีเดียวกัน) — ยังไม่ปลอดภัย ปิดไว้
# ── ผลวัดจริง (2026-08-13) — ทำไมถึงปิด:
#     ยุบไปตำแหน่งบนสุด    196 ชิ้น · แต่ค่าคลาดที่ขอบ 11.65 -> 57.93 (ภาพเพี้ยนยกแผง)
#     รวมแบบไม่ขยับลำดับ  2,428 ชิ้น · คม 573->589 · แต่มี "คราบสีเหลือง" กลางใบไม้
#                          (วัดได้ 5,228 พิกเซลแปลกปลอม จากปกติ 13)
#     ใส่เงื่อนไขกันกรอบทับกันแล้ว -> คราบยังอยู่ 5,228 เท่าเดิม = ยังจับไม่ครบ
# 📌 ต้นเหตุ: SVG เติมสีด้วยกติกา even-odd — ชิ้นที่ซ้อน/ซ้อนในกันเมื่อรวมเป็นชั้นเดียว
#    ส่วนที่ทับกันจะหักล้างกลายเป็นรูโหว่ · การเช็คด้วยกรอบสี่เหลี่ยมหยาบเกินไป
# ✅ ทางที่ถูกคือใช้ shapely รวมรูปทรงจริง (union) แทนการต่อรายการชิ้นเข้าด้วยกัน
#    ซึ่งต้องเขียนใหม่ทั้งบล็อก — ยกไปทำรอบหน้า
# ══════════════════════════════════════════════════════════════════
# 🗺️ PLANAR — ไล่เส้นแบบ "แผนที่ภูมิภาคระนาบ + ขอบร่วม" (2026-08-15)
#
# ⚠️ ทำไมต้องเปลี่ยนโครงสร้าง ไม่ใช่จูนค่า: วิธีเดิมวางชั้นสีทึบซ้อนกัน
#    ขอบระหว่างสองสีจึงถูกลาก **สองครั้ง คนละเส้น** -> ไม่ตรงกันเป๊ะ = "ยึกยือ"
#    และรวมชิ้นสีเดียวกันไม่ได้เพราะ even-odd หักล้าง (ลองแล้ว 3 วิธี พังหมด)
# ✅ วิธีใหม่: แบ่งเป็นภูมิภาคที่ไม่ซ้อนกันเลย ขอบระหว่างสองภูมิภาคเป็นเส้นเดียว
#    ลากครั้งเดียว ฟิตครั้งเดียว สองฝั่งใช้ร่วมกัน (ฝั่งหนึ่งใช้กลับทิศ)
#
# 📊 วัดจริงบนภาพตรา USERS' CHOICE (554 px):
#      ชิ้น 10,608 -> 114 · เนื้อสี 2.16 -> 1.97 · ขอบ 8.49 -> 7.06 · คม 573 -> 599
#      เฉพาะขั้นไล่เส้น 60 วิ -> 20-30 วิ (ไม่ต้องเรียก VTracer + ไม่ต้องให้คะแนน)
#
# 🔒 ทำเฉพาะโหมดไล่สี (grad=True) และเฉพาะเมื่อตรวจเจอพื้นไล่สี
#    โหมดไม่ติ๊ก 🌈 ไม่แตะแม้แต่บรรทัดเดียว (ตรวจด้วย reg.py md5 ทุกรอบ)
# 🛟 พังเมื่อไหร่ตกกลับไปใช้ชั้นเดิมอัตโนมัติ (ครอบ try/except ทั้งก้อน)
# ══════════════════════════════════════════════════════════════════
# 🔬 ตัวขยายภาพ — "เติมพิกเซลให้แน่นขึ้น" ก่อนไล่เส้น (พี่สั่ง 2026-08-18)
# ⚠️ Lanczos ไม่ได้เพิ่มข้อมูล แค่เกลี่ยพิกเซลเดิมให้นุ่ม ตัวอักษรสูง 8 px
#    ขยายไปก็ยังมีข้อมูล 8 px เท่าเดิม
# ✅ โมเดล super-resolution เติมรายละเอียดที่ "ควรจะเป็น" เข้ามาจริง
#    ESPCN/FSRCNN เบามาก (ไฟล์ 40-97 KB · 554->2216 ใช้ 1.1 วินาที)
# ══════════════════════════════════════════════════════════════════
UPSCALER = "lanczos"           # lanczos | espcn | fsrcnn | edsr
# ══════════════════════════════════════════════════════════════════
# 🔬 ผลวัดจริง 2026-08-22 — "ตัวขยายที่ฉลาดกว่า" ไม่ได้ช่วย ตัวที่ช่วยคือ 'ตัวคูณจำนวนเต็ม'
#    ป้าย 554 px ขยายเป็น 2300 px (ค่าคลาด ขอบ / เนื้อ · เวลาขยายภาพ):
#      lanczos  7.66 / 1.97   (ทันที)        ← ใช้ตัวนี้
#      espcn    7.61 / 2.20   (1.1 วิ)       เนื้อภาพแย่ลง · ตัวอักษรจิ๋วติดโทนเขียว
#      fsrcnn   7.54 / 2.22   (1.2 วิ)       เหมือนกัน
#      edsr     — ใช้เวลา ~14 นาที ใช้งานจริงไม่ได้
#    ⚠️ อีกอย่าง: ตัวขยายพวกนี้ต้องใช้ opencv-contrib (cv2.dnn_superres) ซึ่ง
#       requirements.txt ไม่ได้ลง (ลงแค่ opencv-python-headless) -> ถ้าตั้งชื่อพวกนี้
#       โค้ดจะตกกลับไปใช้ lanczos เองอย่างเงียบ ๆ · เก็บไว้เป็นสวิตช์เผื่ออนาคตเท่านั้น
#    ⛔ บทเรียนแพง (2026-08-22): ตอนทดลองไปลง opencv-contrib 5.0 ทับในเครื่องทดสอบ
#       ทำให้ผลของ "โหมดปกติ" เพี้ยนไปจากเดิม (flat_logo 10 -> 11 ชิ้น) ทั้งที่โค้ดไม่ได้แก้
#       -> เวลาตรวจว่า 'เหมือนเดิมทุกไบต์' ต้องใช้ opencv รุ่นเดียวกับที่ขึ้นเซิร์ฟเวอร์เสมอ
# ══════════════════════════════════════════════════════════════════
UPSCALE_DIR = os.environ.get("SR_MODELS", "/opt/sr")
_SRNET = {}


def _upscale(img, nw, nh):
    """ขยายภาพไปที่ขนาดที่ต้องการ — ใช้โมเดล SR ถ้าตั้งไว้และขยายลงตัวพอดี"""
    h, w = img.shape[:2]
    k = str(UPSCALER or "lanczos").lower()
    if k not in ("lanczos", "") and nw >= w and nh >= h:
        try:
            f = min(4, max(2, int(min(nw / max(1, w), nh / max(1, h)))))
            key = (k, f)
            if key not in _SRNET:
                import os as _o
                pth = _o.path.join(UPSCALE_DIR, "%s_x%d.pb" % (k.upper(), f))
                if not _o.path.exists(pth):
                    raise IOError(pth)
                net = cv2.dnn_superres.DnnSuperResImpl_create()
                net.readModel(pth); net.setModel(k, f)
                _SRNET[key] = net
            big = _SRNET[key].upsample(img)
            if big.shape[1] != nw or big.shape[0] != nh:
                big = cv2.resize(big, (nw, nh), interpolation=cv2.INTER_AREA
                                 if big.shape[1] > nw else cv2.INTER_LANCZOS4)
            return big
        except Exception:
            pass
...
SHAPE_GROW = 4.5
# 🎯 ดัน "ขอบของชั้นพื้นไล่สี" ให้มุดเข้าไปใต้ลายลึกแค่ไหน (เท่าของรัศมีฐาน)
#    ขอบชั้นพื้นมาจากเส้นรอบหน้ากาก ซึ่งหยักตามพิกเซลเสมอ — ต้องมุดให้ลึกพอที่
#    ลายด้านบน (วงกลมขาว) จะทับมิด ไม่งั้นขอบหยักจะโผล่พ้นออกมาเป็นรอยแหว่ง
#    วัดจริง 2026-08-23 (ผู้ใช้วงรอบขอบล่างขวาของวงขาว) ความยึกยือของขอบที่เห็นจริง:
#      2.6 เท่า -> sd 0.396 px · สูงสุด 2.42 px   (ต้นฉบับ sd 0.292 · สูงสุด 0.90)
#      4.5 เท่า -> sd 0.240 px · สูงสุด 1.11 px   ← เกือบเท่าต้นฉบับแล้ว และเร็วกว่าด้วย
#    ⚠️ ดันลึกไม่มีผลเสีย เพราะชั้นนี้ถูกวาดเป็นชั้นล่างสุดเสมอ
# ══════════════════════════════════════════════════════════════════
...
#     ปิด      สีเนื้อ 1.72 | คม 508 | 10,239 สี | สีที่โผล่ไม่เกิน 2 ครั้ง 10,043
#     16 สี ✅ สีเนื้อ 2.16 | คม 573 | 16 สี      | สีที่โผล่ไม่เกิน 2 ครั้ง 0
#     10 สี    สีเนื้อ 2.16 | คม 584 | 10 สี      | (คมกว่าแต่เสี่ยงกับงานหลายสี)
# 🎯 จุดสำคัญ: "สีเนื้อ" ขยับแค่ 1.72 -> 2.16 ซึ่ง ΔE ระดับนี้ตาคนแยกไม่ออก
#    = สียังตรงกับต้นฉบับตามที่ผู้ใช้ต้องการ แต่เศษสีขอบ 10,043 สีหายไปหมด
...
def _bez_qd(C, t):
    """ค่า + อนุพันธ์หนึ่ง + อนุพันธ์สอง ในครั้งเดียว (ใช้เลขชี้กำลังร่วมกัน)

    ⚡ เร็วขึ้นเฉย ๆ ไม่ได้เปลี่ยนผลลัพธ์แม้แต่นิดเดียว — สูตรเดิมทุกบรรทัด
       ของเดิมเรียก _bez_at/_bez_d1/_bez_d2 แยกกัน แต่ละตัวคำนวณ t, m ใหม่หมด
       ขั้นขยับพารามิเตอร์ (Newton) เรียกสามตัวนี้ติดกันทุกครั้ง = ทำงานซ้ำ 3 เท่า
       วัดจริงในโปรไฟล์: ขั้นฟิตเบซิเยร์กินเวลา 40 วิ จาก 80 วิ ของงานทั้งชิ้น
    🔒 ลำดับการคูณต้องเหมือนของเดิม "ทุกวงเล็บ" — ถ้าจับคู่คูณใหม่ ผลจะต่างกัน
       ที่หลักทศนิยมสุดท้าย แล้วโหมดปกติจะไม่เท่าเดิมทุกไบต์ตามที่ตกลงกันไว้
       จึงแบ่งใช้ซ้ำได้เฉพาะก้อนที่ 'เขียนเรียงเหมือนกันเป๊ะ' คือ 3*m*m และ 3*t*t
    """
    t = np.asarray(t, float)[:, None]; m = 1.0 - t
    A = 3 * m * m; B = 3 * t * t
    q = (m**3) * C[0] + A * t * C[1] + 3 * m * t * t * C[2] + (t**3) * C[3]
    d1 = A * (C[1] - C[0]) + 6 * m * t * (C[2] - C[1]) + B * (C[3] - C[2])
    d2 = 6 * m * (C[2] - 2 * C[1] + C[0]) + 6 * t * (C[3] - 2 * C[2] + C[1])
    return q, d1, d2
...
def _chord_ud(pts):
    """คืน (พารามิเตอร์ตามคอร์ด, ความยาวแนวจุด) — คำนวณ diff/hypot ครั้งเดียว

    ⚡ ของเดิม _chord_u กับ _fit_cubic ต่างคนต่างเรียก np.diff+np.hypot กับ 'จุดชุดเดียวกัน'
       และ _fit_cubic ถูกเรียกซ้ำถึง 7 ครั้งต่อหนึ่งช่วง (ขยับพารามิเตอร์ 6 รอบ)
       = คำนวณความยาวแนวจุดเดิมซ้ำ 7 ครั้งฟรี ๆ · โปรไฟล์จริง np.diff ถูกเรียก 445,981 ครั้ง
    """
    d = np.hypot(*np.diff(pts, axis=0).T)
    # 🔒 ต้องใช้ d.sum() ไม่ใช่ตัวท้ายของ cumsum — numpy บวกคนละแบบ (จับคู่ vs ไล่ทีละตัว)
    #    ค่าจะต่างกันหลักทศนิยมสุดท้าย ซึ่งพอจะทำให้ผลไม่เท่าเดิมทุกไบต์ได้
    arc = float(d.sum())
    u = np.concatenate([[0.0], np.cumsum(d)])
    return (u / u[-1] if u[-1] > 1e-12 else np.linspace(0.0, 1.0, len(pts))), arc


def _fit_cubic(pts, u, t1, t2, arc=None):
...
def _reparam(pts, u, C):
    ...
    q, d1, d2 = _bez_qd(C, u)          # ⚡ คำนวณครั้งเดียว (ผลเท่าเดิมเป๊ะ ดูหมายเหตุที่ _bez_qd)
    r = q - pts
    num = (r * d1).sum(1)
...
def _schneider(pts, t1, t2, tol, depth=0, max_depth=14):
    ...
    if depth < max_depth:
        ...
        for _ in range(6):                       # ขยับพารามิเตอร์แล้วฟิตใหม่ก่อนคิดจะผ่า
            ...
            # ⚡ ถ้าขยับแล้วได้ค่าชุดเดิมกลับมา (จุดไขว้กัน -> _reparam คืนของเดิม)
            #    รอบต่อ ๆ ไปจะได้ผลเหมือนกันทุกตัวแน่นอน ไม่ต้องวนต่อ (ผลลัพธ์เท่าเดิมเป๊ะ)
            if ub is _prev:
                break
            Cb = _fit_cubic(pts, ub, t1, t2, arc)
...
def vectorize(img_rgba, preset="general", k=None, smooth=None, tol=None, gap=None,
    ...
    if _long > 0 and _long < _tgt_long * 0.95:
        ...
        # 🎯 ปัดตัวคูณให้เป็นจำนวนเต็ม — เหตุผลเต็มอยู่ที่ค่าคงที่ UP_SNAP ด้านบน
        if UP_SNAP and grad and _up > 1.05:
            _ui = (math.ceil(_up - 0.02) if _long <= SMALL_SRC
                   else int(round(_up)))
            _ui = int(max(1, min(int(WORK_UP), _ui)))
            # ถอยลงทีละขั้นจนพอดีทั้ง "แรม" และ "เพดานปัดขึ้น" ของเครื่องเครื่องนี้
            while _ui > 1 and (
                    _long * _ui > _tgt_long * UP_HEAD
                    or (W0 * _ui) * (H0 * _ui) / 1e6 > MAX_MP * 0.55):
                _ui -= 1
            if _ui > 1:
                _up = float(_ui)
        if _up > 1.05:
    ...
    if grad:
        ...
        tol_e = float(np.clip(min(tol_e, 0.15), 0.10, 0.30)) * float(TOL_SCALE)
    ...
    # ══════════════════════════════════════════════════════════════
    # 🧼 ทำความสะอาดจานสี — "ให้เหมือนกราฟิกดราฟท์มือ ไม่ใช่ภาพถ่ายที่วาดด้วยเวกเตอร์"
    # ══════════════════════════════════════════════════════════════
    # ⚠️ ตรวจงานตัวเองแล้วพบ (2026-08-13) — ตัวเลขจากไฟล์ผลลัพธ์จริง:
    #      11,577 ชิ้น · 8,813 สีที่ต่างกัน · 98% ของสีโผล่ไม่เกิน 2 ครั้ง
    #      ใบไม้ 20 ใบที่ควรมี 2-3 สี กลับมี "6,722 เฉด"
    #    ทุกอาการที่ผู้ใช้ทัก (ยึกยือ · ไม่คม · มีเงาจางล้อมทุกชิ้น) คืออาการเดียวกันนี้
    # ⚠️ ต้นเหตุคือ "เป้าหมายผิด": เราจูนด้วยค่าคลาดสีเทียบไฟล์ JPEG ต้นทาง
    #    แต่ JPEG มีรอยบีบอัด + ขอบเกลี่ยสีเต็มไปหมด -> การไล่ค่านั้นให้ต่ำ
    #    = สั่งให้ระบบ "ลอกรอยพวกนั้นมาให้ครบที่สุด" ได้ภาพถ่ายที่วาดด้วยเวกเตอร์
    #    ผู้ใช้ต้องการ "กู้แบบดั้งเดิมที่สะอาดกลับมา" ซึ่งเป็นคนละโจทย์
    # ✅ ยุบสีทั้งหมดให้เหลือจานเดียว แล้วบังคับทุกชิ้นเลือกสีที่ใกล้ที่สุดจากจานนั้น
    #    เงาจางที่ล้อมทุกชิ้น = สีกลางทางระหว่างสองสี พอบังคับให้ "เลือกข้าง"
    #    มันกลายเป็นสีเดียวกับชิ้นข้างเคียงทันที = เงาหายเอง ไม่ต้องไล่ลบทีละอัน
    # 🎯 สีต้องยังใกล้ต้นฉบับ: จานสีสร้างจากสีที่มีอยู่จริงในงาน ถ่วงน้ำหนักตามพื้นที่
    #    -> สีเนื้อใหญ่ ๆ แทบไม่ขยับเลย ที่เปลี่ยนคือเศษสีขอบซึ่งไม่ควรมีตั้งแต่แรก
    if CLEAN_PAL > 0 and grad and len(layers) > CLEAN_PAL:
        try:
            _cl = [L for L in layers if not L.get("grad")]     # ชั้นไล่สีไม่ยุ่ง
            if len(_cl) > CLEAN_PAL:
                _w = np.array([max(1.0, float(L.get("area", L.get("n", 1)))) for L in _cl],
                              np.float64)
                _rgb = np.array([L["rgb"] for L in _cl], np.uint8).reshape(-1, 1, 3)
                _lab = labf(cv2.cvtColor(_rgb, cv2.COLOR_RGB2LAB).reshape(-1, 3))
                # สุ่มตัวอย่างตามพื้นที่ -> สีที่กินพื้นที่มากได้เป็นตัวแทนของจาน
                _rs = np.random.default_rng(seed)
                _pick = _rs.choice(len(_lab), size=min(40000, max(2000, len(_lab) * 8)),
                                   replace=True, p=_w / _w.sum())
                _k = int(min(CLEAN_PAL, max(2, len(set(map(tuple, _lab.round(0)))))))
                _cen, _ = _kmeans_lab(_lab[_pick].astype(np.float32), _k, seed=seed)
                # แต่ละชั้นเลือกสีที่ใกล้ที่สุดในจาน
                _d = ((_lab[:, None, :] - _cen[None, :, :]) ** 2).sum(2)
                _own = _d.argmin(1)
                _cenr = cv2.cvtColor(unlabf(_cen).reshape(-1, 1, 3), cv2.COLOR_LAB2RGB
                                     ).reshape(-1, 3)
                for _i, L in enumerate(_cl):
                    L["rgb"] = tuple(int(v) for v in _cenr[_own[_i]])
                # 🔗 รวมชั้นสีเดียวกันเข้าด้วยกัน — แต่ "ห้ามขยับลำดับซ้อน"
                # ⚠️ ต้นเหตุที่ผู้ใช้เห็นเป็น "ขอบตัวอักษรแหว่งเป็นหยัก" (2026-08-13):
                #    วัดจริงหลังยุบจานสี — ตัวอักษร USERS' CHOICE มี 12 ตัว
                #    แต่สีดำถูกซอยเป็น **1,027 ชิ้น** · ขาว 1,987 ชิ้น · เขียว 771 ชิ้น
                #    แต่ละชิ้นวาดแยกกัน ขอบถูกเกลี่ยกับพื้นหลังคนละครั้ง -> เกิดเส้นรอยต่อ
                #    บาง ๆ ระหว่างชิ้นที่ "สีเดียวกันแท้ ๆ" เต็มไปหมด
                #    ตอนยังมีเงาจางล้อมมันถูกกลบไว้ พอสีแบนสะอาดแล้วเลยโผล่ชัด
                # ⛔ ทางที่ลองแล้วพัง: ยุบทุกชิ้นสีเดียวกันไปที่ "ตำแหน่งบนสุด"
                #    ได้ 196 ชิ้นจริง แต่ค่าคลาดที่ขอบพุ่ง 11.65 -> 57.93
                #    เพราะการขยับลำดับทำให้ชิ้นที่เคยอยู่บนถูกทับ = ภาพเพี้ยนยกแผง
                # ✅ รวมได้เฉพาะเมื่อ "ไม่มีชั้นอื่นที่ทับพื้นที่กันคั่นอยู่ตรงกลาง"
                #    เช็คด้วยกรอบสี่เหลี่ยมล้อม -> รับประกันว่าภาพที่ออกมาเหมือนเดิมเป๊ะ
                def _bb(_L):
                    _xs = []; _ys = []
                    for _it in _L["items"]:
                        _q = _it[1] if isinstance(_it, tuple) else _it
                        try:
                            _arr = np.asarray(_q, np.float64).reshape(-1, 2)
                        except Exception:
                            continue
                        if len(_arr):
                            _xs.append((_arr[:, 0].min(), _arr[:, 0].max()))
                            _ys.append((_arr[:, 1].min(), _arr[:, 1].max()))
                    if not _xs:
                        return None
                    return (min(v[0] for v in _xs), min(v[0] for v in _ys),
                            max(v[1] for v in _xs), max(v[1] for v in _ys))
                _bbs = [_bb(L) for L in layers] if CLEAN_MERGE else []
                _alive = [True] * len(layers)
                _lastc = {}                                   # สี -> ตำแหน่งล่าสุดที่ยังอยู่
                _own = {}                                     # ตำแหน่ง -> กรอบของชิ้นที่รวมไว้
                if not CLEAN_MERGE:
                    _alive = []
                for _i, L in enumerate(layers if CLEAN_MERGE else []):
                    if L.get("grad") or _bbs[_i] is None:
                        _lastc.clear(); continue
                    _key = tuple(L["rgb"])
                    _j = _lastc.get(_key, -1)
                    if _j >= 0:
                        _x0, _y0, _x1, _y1 = _bbs[_i]
                        _ok = True
                        for _m in range(_j + 1, _i):           # มีอะไรทับกรอบนี้คั่นอยู่ไหม
                            if not _alive[_m] or _bbs[_m] is None:
                                continue
                            _a0, _b0, _a1, _b1 = _bbs[_m]
                            if not (_a1 < _x0 or _a0 > _x1 or _b1 < _y0 or _b0 > _y1):
                                _ok = False; break
                        if _ok:
                            # ⚠️ กติกาเติมสีของ SVG คือ even-odd — ถ้าเอาชิ้นที่ "ซ้อนทับกัน"
                            #    มารวมเป็นชั้นเดียว ส่วนที่ทับกันจะหักล้างกันกลายเป็นรูโหว่
                            #    (เจอจริง: มีคราบสีเหลืองโผล่กลางใบไม้) จึงรวมได้เฉพาะชิ้น
                            #    ที่กรอบล้อมไม่ทับกันเองด้วย
                            for _q in _own.get(_j, ()):  # กรอบของชิ้นที่รวมไว้แล้ว
                                _a0, _b0, _a1, _b1 = _q
                                if not (_a1 < _x0 or _a0 > _x1 or _b1 < _y0 or _b0 > _y1):
                                    _ok = False; break
                        if _ok:
                            _P = layers[_j]
                            if not isinstance(_P["items"], list):
                                _P["items"] = list(_P["items"])
                            _P["items"].extend(L["items"])
                            _P["n"] = int(_P.get("n", 1)) + int(L.get("n", 1))
                            _q0, _r0, _q1, _r1 = _bbs[_j]
                            _bbs[_j] = (min(_q0, _x0), min(_r0, _y0),
                                        max(_q1, _x1), max(_r1, _y1))
                            _own.setdefault(_j, [_bb(_P) or (_q0, _r0, _q1, _r1)]
                                            ).append((_x0, _y0, _x1, _y1))
                            _alive[_i] = False
                            continue
                    _lastc[_key] = _i
                if CLEAN_MERGE:
                    layers = [L for _i, L in enumerate(layers) if _alive[_i]]
                _mg, _prev = [], None
                for L in layers:
                    if _prev is not None and _prev is not L and not isinstance(_prev["items"], list):
                        _prev["items"] = list(_prev["items"])
                    if (_prev is not None and not L.get("grad") and not _prev.get("grad")
                            and tuple(L["rgb"]) == tuple(_prev["rgb"])):
                        # ⚠️ ต้องต่อแบบ "ทับที่เดิม" (extend) ห้ามใช้ + สร้างลิสต์ใหม่
                        #    ชั้นมีเป็นหมื่น ถ้าสร้างลิสต์ใหม่ทุกรอบ = คัดลอกซ้ำแบบทวีคูณ
                        #    (ลองแล้วโปรเซสถูกฆ่าเพราะแรมหมดทันที)
                        if _prev["items"] is L["items"]:
                            continue
                        _prev["items"].extend(L["items"])
                        _prev["n"] = int(_prev.get("n", 1)) + int(L.get("n", 1))
                        continue
                    _mg.append(L); _prev = L
                layers = _mg
        except Exception:
            pass                                     # พังเมื่อไหร่ใช้ของเดิม ไม่ทำให้งานล่ม
    # ══════════════════════════════════════════════════════════════
    # 🗺️ แทนที่ "ชั้นลาย" ทั้งหมดด้วยแผนที่ภูมิภาคระนาบ + ขอบร่วม
    #    ชั้นพื้นไล่สี (grad) ไม่แตะเลย — ใช้ของเดิมเป๊ะทุกค่า
    # ══════════════════════════════════════════════════════════════
    if _planar_on:
        try:
            try:
            ...
            _pl = _VP.planar_layers(img, _pkeep, seed=seed, progress=_pg,
                                    ...
                                    snap=(not _smr), upscale=float(sc))
            if _pl:
                # ══════════════════════════════════════════════════════
            ...
            PLANAR_NOTE[0] = "planar ล้มเหลว: %s — ใช้ชั้นเดิม" % _e9
