...
SVG_LONG = 5000        # ขนาดที่ประกาศไว้ในไฟล์ SVG (ด้านยาว, px) · 0 = ใช้ขนาดเท่าต้นฉบับ
...
def to_svg(res, scale=1.0, background=True):
    ...
    # ══════════════════════════════════════════════════════════════
    # 🔍 ประกาศขนาดไฟล์ให้ "ใหญ่ไว้ก่อน" — เรขาคณิตข้างในไม่เปลี่ยนแม้แต่จุดเดียว
    # ⚠️ ต้นเหตุที่ผู้ใช้บอกว่า "SVG ก็ไม่ชัด" (จับได้ 2026-08-13 จากหัวหน้าต่าง
    #    โปรแกรมดูรูป: "งานแว่น (6).svg SVG 554x554"):
    #    เราเขียน width/height ตามขนาดไฟล์ต้นทางเป๊ะ ๆ คือ 554 x 554
    #    โปรแกรมดูรูป/โปรแกรมออกแบบจำนวนมาก "วาดครั้งเดียวที่ขนาดที่ประกาศ"
    #    แล้วค่อยขยายภาพที่วาดไว้ให้ดู -> ซูมเท่าไหร่ก็เบลอ ทั้งที่ข้างในเป็นเวกเตอร์แท้
    #    (ยิ่งไฟล์ต้นทางเล็ก ยิ่งเห็นชัด — 554 px ซูม 10 เท่าเหลือความละเอียดจริง 1/10)
    # ✅ ประกาศด้านยาวเป็น ~5000 px แต่คง viewBox เป็นพิกัดเดิม
    #    = รูปเดิมทุกประการ แต่โปรแกรมที่วาดครั้งเดียวจะวาดที่ 5000 px ให้เอง
    #    ไฟล์ไม่ใหญ่ขึ้นเลยแม้แต่ไบต์เดียว (เปลี่ยนแค่ตัวเลขสองตัวบนหัวไฟล์)
    # ══════════════════════════════════════════════════════════════
    if SVG_LONG > 0 and max(ow, oh) > 0:
        _k = float(SVG_LONG) / float(max(ow, oh))
        if _k > 1.0:
            ow, oh = int(round(ow * _k)), int(round(oh * _k))
