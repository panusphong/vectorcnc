...
def _extract_subpaths_pdf(path, filetype=None):
    ...
    try:
        ...
        #    ⚠️ ห้ามทิ้งแบบ 'ล้นนิดเดียวก็ทิ้ง' เด็ดขาด — ตัวอักษรตัวริมขวาสุดล้นขอบนิดเดียว
        #       จะโดนทิ้งทั้งตัว (เคสตัว 'u' ของคำว่า You หายไปทั้งตัว)
        #    เกณฑ์ที่ใช้จริง 2 ข้อ ต้องเข้าข้อใดข้อหนึ่งถึงจะทิ้ง:
        #      1) เส้นนั้น 'ใหญ่เกินหน้ากระดาษ' (บวมจากการสเกล = ขยะแน่นอน)
        ...
        def _inpage(sp):
            try:
                ...
                if (x0 >= R.x0 - _PAD and y0 >= R.y0 - _PAD
                        and x1 <= R.x1 + _PAD and y1 <= R.y1 + _PAD):
                    return True                       # อยู่ในหน้ากระดาษ -> เก็บ (ทางลัดปกติ)
                bw = x1 - x0; bh = y1 - y0
                if bw > W * 1.05 or bh > H * 1.05:
                    return False                      # 1) บวมเกินหน้ากระดาษ = ขยะจากการสเกล
                _iw = max(0.0, min(x1, R.x1) - max(x0, R.x0))
                _ih = max(0.0, min(y1, R.y1) - max(y0, R.y0))
                _ba = max(1e-6, bw * bh)
                return (_iw * _ih) / _ba >= 0.35      # 2) ทับหน้ากระดาษ >=35% -> ยังเป็นงานจริง เก็บไว้
            except Exception:
                return True
