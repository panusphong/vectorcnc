...
def draw_finished(g, outpath, prof=None):
    OVAL, BG, LET, LEAF, RINGS, KW = g['OVAL'], g['BG'], g['LET'], g['LEAF'], g['RINGS'], g['KW']
    _base = (prof is None) or prof.get('base', True); _frame = (prof is None) or prof.get('frame', True)
    ...
    def wall(coords, zb, zt, fill, op=1.0, st='none', sw=0):
        ...
        return out
    PH = (50*MM if _base else 0); LH = 70*MM
    defs = ('<defs>'
...
# ================= EXPLODED =================
def draw_exploded(g, outpath, prof=None):
    OVAL, BG, LET, LEAF, RINGS, KW, WLEAF = g['OVAL'], g['BG'], g['LET'], g['LEAF'], g['RINGS'], g['KW'], g['WLEAF']
    _base = (prof is None) or prof.get('base', True)
    _frame = (prof is None) or prof.get('frame', True)
    _acr = (prof is None) or (prof.get('face') == 'acrylic') or bool(prof.get('backer'))
    ...
    if _base: PL.append(dim(0, PH, 'outer edge 5cm', '#e11d48'))
    PL.append(dim(z2, z2+RH, 'letters 7cm (raised)', '#2563eb'))
...
# ================= LED PLAN (type-aware) =================
def draw_led_plan(g, outpath, prof=None):
    OVAL, LET, LEAF, RINGS = g['OVAL'], g['LET'], g['LEAF'], g['RINGS']; W, Hh = g['W'], g['H']
    _front = (prof is None) or ('front' in prof.get('led', ''))
    _halo = (prof is None) or ('halo' in prof.get('led', ''))
    _neon = (prof is not None) and (prof.get('led') == 'neon')
    _none = (prof is not None) and (prof.get('led') == 'none')
    ...
    if _neon:
        for p in LET+LEAF: ringL(p, led, col='#e11d8f', w=3.4)
    if _halo and not _front:
        # halo รอบตัวอักษร (เรืองหลัง)
        for p in LET+LEAF:
            hp = p.buffer(6)
            ringL(hp, led, col='#f59e0b', w=2.6)
    q += led
    if _none:
        label = 'NO LED — ยกขอบไม่มีไฟ (4.1)'; col = '#64748b'
    elif _neon:
        label = 'NEON FLEX — เดินเส้นตามตัวอักษร (เซาะร่อง 8mm)'; col = '#be185d'
    elif _front and _halo:
        label = 'FRONT-LIT (letters+leaves+branch) + BACK halo'; col = '#7a4a12'
    elif _front:
        label = 'FRONT-LIT — fill letters (box light)'; col = '#7a4a12'
    else:
        label = 'BACK-LIT halo — รอบตัวอักษร (เรืองหลัง 4.2)'; col = '#a16207'
    q.append(f'<g font-family="sans-serif"><rect x="18" y="14" width="360" height="28" rx="7" fill="#fffdf7" stroke="#e2cea5"/>'
             f'<text x="30" y="32" font-size="12" fill="{col}">{label}</text></g>')
    q.append(f'<text x="24" y="{Hh+24:.0f}" font-family="sans-serif" font-size="12.5" fill="#334155" font-weight="700">LED PLAN — {label}</text>')
    q.append('</svg>')
    return _png('\n'.join(q), outpath, 900)


# ================= NESTING SPLIT (type-aware) =================
def draw_nesting_split(g, outpath, prof=None):
    """แยกการตัดตามวัสดุ: โลหะ (แผ่นฐาน/หน้าอักษรตัน/คิ้ว) | อะคริลิค (หน้า/แผ่นรอง)"""
    OVAL, LET, KW, RINGS, WLEAF = g['OVAL'], g['LET'], g['KW'], g['RINGS'], g['WLEAF']
    W, Hh = g['W'], g['H']; PANEL = W
    _base = (prof is None) or prof.get('base', True)
    _frame = (prof is None) or prof.get('frame', True)
    _face_metal = (prof is not None) and (prof.get('face') == 'metal_solid')
    _face_acr = (prof is None) or (prof.get('face') == 'acrylic')
    _backer = (prof is not None) and bool(prof.get('backer'))
    _has_acr = _face_acr or _backer
    ...
    scale = 0.46
    def box(x0, wpx, title, sub, content, fill):
        return (f'<g transform="translate({x0},40)">'
                f'<rect x="0" y="0" width="{wpx:.0f}" height="{Hh*scale+70:.0f}" rx="12" fill="{fill}" stroke="#cbd5e1"/>'
    ...
    metal = ''
    parts = []
    if _base: metal += grp([OVAL], '#fde7ec', '#e11d48'); parts.append('แผ่นฐาน')
    if _face_metal: metal += grp(LET, '#eef2f7', '#64748b'); parts.append('หน้าอักษรตัน')
    if _frame: metal += grp(KW, '#fff3e0', '#d97706'); parts.append('คิ้ว')
    if not metal:  # เผื่อกรณีไม่มีโลหะเลย
        metal = grp([OVAL], '#f1f5f9', '#94a3b8'); parts.append('—')
    metal_sub = ' + '.join(parts) + ' — DXF #1'

    CW = PANEL; CH = Hh*scale+130
    if _has_acr:
        _sil = acr_sil(g)
        acr = grp(_sil, '#e3f4ec', '#059669') + line(LET + RINGS + WLEAF, '#8fd3ac', 0.8, 0.6)
        acr_sub = ('แผ่นรองหลัง (halo)' if (_backer and not _face_acr) else 'หน้าอะคริลิค silhouette') + ' — DXF #2'
        body = (box(10, PANEL*0.5-30, 'CUT: METAL', metal_sub, metal, '#fffafb')
                + box(PANEL*0.5+10, PANEL*0.5-30, 'CUT: ACRYLIC', acr_sub, acr, '#fafffb'))
        foot = 'NESTING — โลหะ และ อะคริลิค ตัดคนละแผ่น (2 DXF)'
    else:
        body = box(10, PANEL-40, 'CUT: METAL', metal_sub, metal, '#fffafb')
        foot = 'NESTING — งานนี้ตัดโลหะอย่างเดียว (1 DXF)'
    svg = (f'<svg xmlns="http://www.w3.org/2000/svg" width="{CW:.0f}" height="{CH:.0f}" viewBox="0 0 {CW:.0f} {CH:.0f}">'
           f'<rect width="100%" height="100%" fill="#fff"/>' + body
           + f'<text x="16" y="{CH-14:.0f}" font-family="sans-serif" font-size="12" fill="#5a6b8c" font-weight="700">{foot}</text></svg>')
